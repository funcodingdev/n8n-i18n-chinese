...
import { eo as AGENT_SKILL_INSTRUCTIONS_MAX_LENGTH, no as AGENT_SKILL_REFERENCE_CONTENT_MAX_BYTES } from "./src-B2jBAc9z.js";
...
import { n as AgentChipButton_default, t as normalizeAgentSkillForSave } from "./agentSkill-Bpow7n7L.js";
...
function useAgentSkillImport() {
	async function importSkillFiles(files) {
		...
		for (const entry of fileEntries) {
			...
			if (entry.file.size > 65536) throw new AgentSkillImportError("agents.builder.skills.import.referenceTooLarge");
			seenPaths.add(relativePath);
			const content = await readFileText(entry.file);
			const bytes = new TextEncoder().encode(content).byteLength;
			if (bytes > 65536) throw new AgentSkillImportError("agents.builder.skills.import.referenceTooLarge");
			totalReferenceBytes += bytes;
			if (totalReferenceBytes > 262144) throw new AgentSkillImportError("agents.builder.skills.import.referencesTooLarge");
			references.push({
				path: relativePath,
				content
			});
...
var AgentSkillViewer_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		const instructionsError = computed(() => {
			...
			if (instructionsBytes.value > 65536) return i18n.baseText("agents.builder.skills.validation.instructionsMaxLength", { interpolate: { max: String(AGENT_SKILL_INSTRUCTIONS_MAX_LENGTH) } });
			return "";
		});
		const instructionsValid = computed(() => Boolean((props.skill.instructions ?? "").trim()) && !instructionsError.value);
		const referenceBytes = (reference) => utf8Bytes(reference.content);
		const invalidReferences = computed(() => (props.skill.references ?? []).filter((reference) => !reference.content.trim() || referenceBytes(reference) > 65536));
		const totalReferenceBytes = computed(() => (props.skill.references ?? []).reduce((total, reference) => total + referenceBytes(reference), 0));
		const referencesTotalError = computed(() => {
			if (totalReferenceBytes.value <= 262144) return "";
		...
		const instructionsByteCount = computed(() => i18n.baseText("agents.builder.skills.instructions.byteCount", { interpolate: {
		...
		const selectedReferenceByteCount = computed(() => i18n.baseText("agents.builder.skills.references.byteCount", { interpolate: {
			count: (selectedReference.value ? referenceBytes(selectedReference.value) : 0).toLocaleString(),
		...
		const selectedReferenceError = computed(() => {
			...
			if (referenceBytes(reference) > 65536) return i18n.baseText("agents.builder.skills.references.contentMaxBytes", { interpolate: { max: AGENT_SKILL_REFERENCE_CONTENT_MAX_BYTES.toLocaleString() } });
			return "";
		});
			...
			return openBlock(), createElementBlock("div", {
				...
				createBaseVNode("div", { class: normalizeClass([_ctx.$style.field, _ctx.$style.instructionsField]) }, [createVNode(unref(N8nInputLabel_default), {
					...
					default: withCtx(() => [createVNode(unref(MarkdownEditor_default), {
						...
						createVNode(unref(N8nText_default), {
							...
							default: withCtx(() => [createTextVNode(toDisplayString(instructionsByteCount.value), 1)]),
							_: 1
						})
					], 2)]),
					_: 1
				...
				default: withCtx(() => [createVNode(unref(MarkdownEditor_default), {
					...
					createVNode(unref(N8nText_default), {
						...
						default: withCtx(() => [createTextVNode(toDisplayString(selectedReferenceByteCount.value), 1)]),
						_: 1
					})
				], 2)]),
				_: 1
...
var AgentSkillModal_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const validationErrors = computed(() => {
			...
			else if (new TextEncoder().encode(skill.value.instructions).byteLength > 65536) errors.instructions = i18n.baseText("agents.builder.skills.validation.instructionsMaxLength", { interpolate: { max: String(AGENT_SKILL_INSTRUCTIONS_MAX_LENGTH) } });
