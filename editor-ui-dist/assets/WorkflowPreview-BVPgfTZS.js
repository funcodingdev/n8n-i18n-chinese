import { $ as openBlock, C as createBaseVNode, E as createElementBlock, Gt as unref, It as ref, N as defineComponent, S as computed, T as createCommentVNode, X as onMounted, gt as watch, j as createVNode, q as onBeforeUnmount, vn as normalizeClass } from "./vue.runtime.esm-bundler-CiTcVoZc.js";
import { s as useI18n } from "./src-LyB2u_3c.js";
import { Ht as N8nLoading_default, Ya as N8nIcon_default } from "./src-BY3LHeey.js";
...
var _hoisted_1 = ["src", "data-ndv-open"];
var WorkflowPreview_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		const iframeSrc = computed(() => {
			...
			if (!props.canOpenNDV) params.set("canOpenNDV", "false");
		...
		let lastSentWorkflow;
		let lastSentExecutionId;
		...
		const loadWorkflow = () => {
			try {
				...
				if (props.workflow === lastSentWorkflow) return;
		...
		const loadExecution = () => {
			try {
				...
				if (props.executionId === lastSentExecutionId) return;
		...
		watch(() => props.mode, () => {
			lastSentWorkflow = void 0;
			lastSentExecutionId = void 0;
			if (showPreview.value) {
				if (props.mode === "workflow") loadWorkflow();
				else if (props.mode === "execution") loadExecution();
			}
		...
		watch(() => props.executionId, () => {
			if (props.mode === "execution" && props.executionId && ready.value) loadExecution();
		});
		watch(() => props.workflow, (newWorkflow, oldWorkflow) => {
			if (!ready.value) return;
			if (oldWorkflow && oldWorkflow !== newWorkflow) sendResetWorkflow();
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", { class: normalizeClass(_ctx.$style.container) }, [__props.loaderType === "image" && !showPreview.value ? (openBlock(), createElementBlock("div", {
				...
				src: iframeSrc.value,
				"data-ndv-open": nodeViewDetailsOpened.value || void 0,
