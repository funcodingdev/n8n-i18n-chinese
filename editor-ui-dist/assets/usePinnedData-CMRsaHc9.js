import { Gt as unref, S as computed } from "./vue.runtime.esm-bundler-Bs4WIMNP.js";
import { x as useI18n } from "./_MapCache-B3VhjNx3.js";
import { t as useToast } from "./useToast-9MJyC6NG.js";
import { L as useNodeTypesStore, Ot as useDataSchema, ci as pinDataToExecutionData, si as getPinDataSize, w as injectWorkflowDocumentStore } from "./workflows.store-HMd7zwNZ.js";
import { Er as getNodeOutputs, Mn as isTrimmedNodeExecutionData, Rr as isSubNodeType, St as MAX_EXPECTED_REQUEST_SIZE, ai as NodeConnectionTypes, gi as jsonStringify, hi as jsonParse, wt as MAX_WORKFLOW_SIZE } from "./src-Bd4giLF5.js";
import { t as useRootStore } from "./useRootStore-ChBg8j_7.js";
import { kn as PIN_DATA_NODE_TYPES_DENYLIST } from "./constants-CAglk9RE.js";
import { n as useTelemetry, o as useUIStore } from "./posthog.store-Cd_RzoQK.js";
import { _ as toMegaBytes, g as stringSizeInBytes } from "./typesUtils-C0qTIOD0.js";
...
function usePinnedData(node, options = {}) {
	...
	const data = computed(() => {
		...
		return pinDataToExecutionData(workflowDocumentStore.value.pinnedDataByNodeName)[targetNode.name];
	});
	...
	function canPinNode(checkDataEmpty = false, outputIndex) {
		...
		const outputs = expression ? getNodeOutputs({ expression }, targetNode, nodeType).map((output) => typeof output === "string" ? { type: output } : output) : [];
	...
	function isValidSize(data) {
		...
		const { pinData: _pinData, ...workflowObjectWithoutPinData } = workflowDocumentStore.value?.getSnapshot() ?? {};
		const currentPinData = workflowDocumentStore.value?.pinnedDataByNodeName ?? {};
	...
	function setData(data, source) {
		...
		if (Array.isArray(data) && isTrimmedNodeExecutionData(data)) {
			onSetDataError({
				errorType: "trimmed-data",
				source
			});
		...
		if (workflowDocumentStore.value) {
			...
			if ((workflowDocumentStore.value.pinnedDataByNodeName[nodeName] ?? []).length > 0) workflowDocumentStore.value.touchPinnedDataLastUpdatedAt(nodeName);
