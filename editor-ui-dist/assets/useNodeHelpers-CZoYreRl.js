...
import { t as useLoadingService } from "./useLoadingService-BUe8OLS5.js";
import { t as createEventBus } from "./event-bus-RxIHoNRV.js";
import { Bi as isString, Et as useCredentialsStore, H as useNodeTypesStore, M as assignNodeId, O as injectWorkflowDocumentStore, ar as hasProxyAuth, er as getNodeSubtitle, v as injectWorkflowExecutionStateStore } from "./workflows.store-DFwITVQx.js";
import { Ai as getNodeParameters, Di as getConnectionTypes, Oi as getNodeInputs, Ti as displayParameterPath, Ui as makeNodeName, Un as classifyTriggerIdentity, Wi as mergeIssues, fi as getParentNodes, ji as getNodeParametersIssues, la as NodeConnectionTypes, pi as getChildNodes, qi as nodeIssuesToString } from "./src-C4KdskeD.js";
...
import { y as isObject } from "./typesUtils-DYmNZ-mU.js";
...
var COMMANDS = /* @__PURE__ */ function(COMMANDS) {
	...
	COMMANDS["ADD_NODE_GROUP"] = "addNodeGroup";
	COMMANDS["REMOVE_NODE_GROUP"] = "removeNodeGroup";
	COMMANDS["UPDATE_NODE_GROUP"] = "updateNodeGroup";
	return COMMANDS;
}({});
...
function groupsAreEqual(a, b) {
	return a.id === b.id && a.name === b.name && a.description === b.description && a.nodeIds.length === b.nodeIds.length && a.nodeIds.every((nodeId, index) => nodeId === b.nodeIds[index]);
}
var AddNodeGroupCommand = class AddNodeGroupCommand extends Command {
	group;
	constructor(group, timestamp) {
	...
	isEqualTo(anotherCommand) {
		return anotherCommand instanceof AddNodeGroupCommand && anotherCommand.group.id === this.group.id;
	}
	async revert() {
		return await new Promise((resolve) => {
...
var RemoveNodeGroupCommand = class RemoveNodeGroupCommand extends Command {
	group;
	constructor(group, timestamp) {
	...
	isEqualTo(anotherCommand) {
		return anotherCommand instanceof RemoveNodeGroupCommand && anotherCommand.group.id === this.group.id;
	}
	async revert() {
		return await new Promise((resolve) => {
...
var UpdateNodeGroupCommand = class UpdateNodeGroupCommand extends Command {
	before;
	after;
	constructor(before, after, timestamp) {
	...
	isEqualTo(anotherCommand) {
		return anotherCommand instanceof UpdateNodeGroupCommand && groupsAreEqual(anotherCommand.before, this.before) && groupsAreEqual(anotherCommand.after, this.after);
	}
	async revert() {
		return await new Promise((resolve) => {
...
function useNodeHelpers() {
	...
	*/
	...
	const privateCredentialTriggerWarning = computed(() => {
		const triggers = workflowDocumentStore.value.workflowTriggerNodes.filter((trigger) => !trigger.disabled);
		if (triggers.length === 0) return null;
		const resolverId = workflowDocumentStore.value.settings?.credentialResolverId;
		const isSystemResolver = !resolverId || resolverId === "system-n8n";
		const isBlocking = (trigger) => {
			const { providesN8nIdentity, providesExternalIdentity } = classifyTriggerIdentity(trigger.type, trigger.parameters);
		...
		if (blockingTriggers.length === triggers.length) return {
			reachable: null,
			isSystemResolver
		};
		...
		for (const trigger of blockingTriggers) {
			...
			for (const child of getChildNodes(bySource, trigger.name, NodeConnectionTypes.Main)) reachable.add(child);
		}
		for (const nodeName of [...reachable]) for (const subNode of getParentNodes(byDestination, nodeName, "ALL_NON_MAIN")) reachable.add(subNode);
		return {
			reachable,
			isSystemResolver
		};
	...
	function collectPrivateCredentialIssues(node, foundIssues) {
		...
		for (const [credTypeName, details] of Object.entries(node.credentials ?? {})) {
			...
			if (!credentialsStore.getCredentialById(details.id)?.isResolvable) continue;
			const messageKey = warning.isSystemResolver ? "nodeIssues.credentials.privateRequiresIdentityTriggerWithFormAndWebhook" : "nodeIssues.credentials.privateRequiresIdentityExtractor";
	...
	*/
	...
	function getBinaryData(runDataOfNode, outputIndex, connectionType = NodeConnectionTypes.Main) {
...
//#endregion
export { useCanvasStore as _, AddNodeGroupCommand as a, EnableNodeToggleCommand as c, RemoveNodeCommand as d, RemoveNodeGroupCommand as f, historyBus as g, UpdateNodeGroupCommand as h, AddNodeCommand as i, MoveNodeCommand as l, ReplaceNodeParametersCommand as m, useHistoryStore as n, BulkCommand as o, RenameNodeCommand as p, AddConnectionCommand as r, Command as s, useNodeHelpers as t, RemoveConnectionCommand as u };
