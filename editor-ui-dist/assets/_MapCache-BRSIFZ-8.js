...
var zh_CN_default = {
	...
	"workflowSettings.saveExecutionProgressOptions.save": "@:_reusableBaseText.save",
	"workflowSettings.redactionPolicy": "数据脱敏政策",
	"workflowSettings.redactionPolicy.options.none": "不进行脱敏",
	"workflowSettings.redactionPolicy.options.all": "隐藏所有执行记录",
	"workflowSettings.redactionPolicy.options.nonManual": "隐藏非手动执行记录",
	"workflowSettings.helpTexts.redactionPolicy": "控制如何对执行数据进行脱敏处理。“全部脱敏”会对所有执行的数据进行脱敏处理。“脱敏非手动”仅对生产环境（非手动触发）的执行数据进行脱敏处理。",
	...
	"credentialResolverEdit.confirmMessage.deleteResolver.message": "您确定要删除凭证解析器“{savedResolverName}”吗？",
