...
import { Ju as ChatHubToolContextKey, Na as LOCAL_STORAGE_SKIP_DISCONNECT_CONFIRM, _i as AI_GATEWAY_TOP_UP_MODAL_KEY, bn as KEEP_AUTH_IN_NDV_FOR_NODES, bu as getResourcePermissions } from "./constants-DDbk7QLF.js";
...
import { h as assert } from "./useRootStore-Cls9JR4X.js";
import { f as useUIStore, p as useTelemetry, t as useUsersStore } from "./users.store-C8AeNcFl.js";
import { y as isEmpty } from "./workflowsList.store-Ci-FEnCL.js";
...
import { t as useAiGateway } from "./useAiGateway-DHji8ctL.js";
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", {
				...
				"text",
				"hover-text"
			])) : createCommentVNode("", true)], 10, _hoisted_2), createBaseVNode("button", {
				type: "button",
				role: "radio",
				"aria-checked": !__props.aiGatewayEnabled,
				disabled: __props.readonly,
		...
		watch(credentialTypesNodeDescriptionDisplayed, (types) => {
			...
			if (allOptions.length === 0) {
				if (aiGateway.isEnabled.value) {
					for (const { type } of types) if (aiGateway.isCredentialTypeSupported(type.name) && aiGateway.isNodeTypeVersionSupported(node.value.type, node.value.typeVersion)) onAiGatewaySelector(type.name, true, false);
				}
				return;
			}
