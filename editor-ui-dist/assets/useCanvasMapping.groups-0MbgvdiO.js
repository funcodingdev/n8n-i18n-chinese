...
import { Bi as CANVAS_NODE_GROUP_HANDLE_RIGHT, Gi as createCanvasGroupNodeId, Rr as DEFAULT_NODE_SIZE, Vi as CANVAS_NODE_GROUP_TYPE, br as applyOffset, wr as createCanvasConnectionId, zi as CANVAS_NODE_GROUP_HANDLE_LEFT } from "./workflowDocument.store-COBWkqWz.js";
...
function mapGroupsToVueFlowNodes({ allGroups, getNodeById, getNodeDisplaySize, getGroupVisualOffset, isGroupCollapsed, readOnly, getNodeExecutionSnapshot }) {
	...
	for (const group of allGroups) {
		...
		const memberNodes = group.nodeIds.map(getNodeById).filter((node) => node !== void 0);
		const data = {
			group,
			nodesRect,
