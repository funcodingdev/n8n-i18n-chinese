...
import { Qr as v4_default, Tr as useToast, at as useCredentialsStore, en as useTelemetry, js as useSettingsStore } from "./users.store-D86YL4LK.js";
import { Ya as deepCopy, _i as emptyChatModelsResponse, cl as EnterpriseEditionFeature, gi as chatHubVectorStoreProviderSchema, hi as chatHubProviderSchema, li as PROVIDER_CREDENTIAL_TYPE_MAP, mi as chatHubMessageWithButtonsSchema, pi as chatHubLLMProviderSchema, ui as VECTOR_STORE_PROVIDER_CREDENTIAL_TYPE_MAP } from "./constants-bR1gDMyV.js";
import { a as STORES, c as ResponseError, d as makeRestApiRequest, j as defineStore, r as useRootStore } from "./_baseOrderBy-Bnq6lF9G.js";
import { t as retry } from "./retry-Coa047ux.js";
import { l as CHAT_VIEW, s as CHAT_SESSIONS_PAGE_SIZE } from "./constants-CrjCGXcB.js";
...
const useChatStore = defineStore(STORES.CHAT_HUB, () => {
	...
	async function createCustomAgent(payload, files, credentials) {
		...
		if (files.length > 0) customAgent = await uploadAgentFilesApi(rootStore.restApiContext, customAgent.id, files);
	...
	async function updateCustomAgent(agentId, payload, newFiles, fileKnowledgeIdsToDelete, credentials) {
		...
		if (newFiles.length > 0) await uploadAgentFilesApi(rootStore.restApiContext, agentId, newFiles);
		const customAgent = await fetchAgentApi(rootStore.restApiContext, agentId);
...
export { workflowAgentDefaultIcon as C, fetchWorkflowExecutionStatus as D, fetchChatModelsApi as E, revokeDynamicCredential as O, unflattenModel as S, buildChatAttachmentUrl as T, isRegenerable as _, createFakeAgent as a, splitMarkdownIntoChunks as b, findOneFromModelsResponse as c, getAgentRoute as d, groupConversationsByDate as f, isLlmProviderModel as g, isLlmProvider as h, collectChatArtifacts as i, updateSemanticSearchSettingsApi as k, flattenModel as l, isEditable as m, DEFAULT_SEMANTIC_SEARCH_SETTINGS as n, enrichMimeTypesWithExtensions as o, isAllowedModel as p, EMBEDDINGS_NODE_TYPE_MAP as r, filterAndSortAgents as s, useChatStore as t, fromStringToModel as u, isWaitingForApproval as v, authorizeDynamicCredential as w, stringifyModel as x, personalAgentDefaultIcon as y };
