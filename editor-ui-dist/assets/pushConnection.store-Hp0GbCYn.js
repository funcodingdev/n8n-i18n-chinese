...
import { O as createHeartbeatMessage } from "./src-B2jBAc9z.js";
...
var useEventSourceClient = (options) => {
	...
	const onConnectionLost = () => {
		...
		isConnected.value = false;
		if (eventSource.value?.readyState === EventSource.CLOSED) reconnectTimer.scheduleReconnect();
	};
	const onMessage = (event) => {
		options.onMessage(event.data);
	};
...
var usePushConnectionStore = defineStore(STORES.PUSH, () => {
	...
	*/
	...
	const pushConnect = () => {
		...
		if (connectionOwnerCount !== 1) return;
		isConnectionRequested.value = true;
		isConnecting.value = true;
		client.value.connect();
	};
	const pushDisconnect = () => {
		if (connectionOwnerCount === 0) return;
		connectionOwnerCount--;
