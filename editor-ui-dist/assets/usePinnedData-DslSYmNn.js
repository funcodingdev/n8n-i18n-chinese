...
import { Br as createWorkflowDocumentId, Gr as pinDataToExecutionData, Hr as injectWorkflowDocumentStore, It as useTelemetry, Sr as useExternalHooks, Ta as toMegaBytes, Ur as useWorkflowDocumentStore, Wr as getPinDataSize, br as useToast, lt as useDataSchema, r as useUIStore, s as useWorkflowsStore, w as useNodeTypesStore, wa as stringSizeInBytes } from "./users.store-CgsgTstk.js";
import { Ka as jsonParse, Zn as PIN_DATA_NODE_TYPES_DENYLIST, _c as MAX_PINNED_DATA_SIZE, co as NodeConnectionTypes, fa as getNodeOutputs, gc as MAX_EXPECTED_REQUEST_SIZE, qa as jsonStringify, vc as MAX_WORKFLOW_SIZE, wa as isSubNodeType } from "./constants-DEPRN4fc.js";
...
function usePinnedData(node, options = {}) {
	...
	const workflowDocumentStore = injectWorkflowDocumentStore() ?? (workflowsStore.workflowId ? useWorkflowDocumentStore(createWorkflowDocumentId(workflowsStore.workflowId)) : null);
	...
	const data = computed(() => {
		...
		if (!targetNode || !workflowDocumentStore) return void 0;
	...
	function isValidSize(data$1) {
		...
		const currentPinData = workflowDocumentStore?.pinData ?? {};
	...
	function setData(data$1, source) {
		...
		if (workflowDocumentStore) {
			...
			if ((workflowDocumentStore.pinData[nodeName] ?? []).length > 0 && workflowsStore.nodeMetadata[nodeName]) workflowsStore.nodeMetadata[nodeName].pinnedDataLastUpdatedAt = Date.now();
			workflowDocumentStore.pinNodeData(nodeName, data$1);
