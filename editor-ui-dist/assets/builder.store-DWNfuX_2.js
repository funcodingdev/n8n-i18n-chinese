...
import { Ft as readonly, It as ref, P as getCurrentInstance, R as inject, S as computed, gt as watch } from "./vue.runtime.esm-bundler-DYHsQBZB.js";
import { s as useI18n } from "./src-BBBtAtId.js";
import { H as useDebounceFn, b as useLocalStorage } from "./dist-CYNfKCbA.js";
import { t as useMessage } from "./useMessage-CMftD8l5.js";
import { t as require_lib } from "./lib-DWs31ikH.js";
import { c as useRoute, l as useRouter } from "./vue-router-D2dKRIiV.js";
import { A as useWorkflowDocumentStore, Ft as useAIAssistantHelpers, H as useNodeTypesStore, Mt as getBuilderCredits, Ni as isDebouncedFunction, O as injectWorkflowDocumentStore, Ot as chatWithBuilder, Pt as truncateBuilderMessages, T as createWorkflowDocumentId, en as useWorkflowHelpers, fn as useNDVStore, jt as getAiSessions, kt as clearBuilderSession, nn as useWorkflowId, t as useWorkflowsStore, tn as useRouteWorkflowId, xn as useSourceControlStore, y as useWorkflowExecutionStateStore } from "./workflows.store-Dti-Ds2u.js";
import { Jn as validateNodeSelectionForGrouping, Nt as getResourcePermissions, ha as dedupe, pa as isExpression } from "./src-BrbN-vPn.js";
import { Zn as ResponseError, a as getWorkflowVersion, gn as getFirstAdoptionDate, i as getWorkflowHistory, o as getWorkflowVersionsByIds, r as getPublishTimeline, s as updateWorkflowHistoryVersion, sr as STORES, t as useRootStore, ur as defineStore, xt as updateNpsSurveyState } from "./useRootStore-B-A2Ktsc.js";
...
import { Di as LOCAL_STORAGE_BROWSER_NOTIFICATION_METADATA, Ga as MERGE_ASK_BUILD_EXPERIMENT, Na as FOCUSED_NODES_EXPERIMENT, ai as MODAL_CLOSE, ci as NPS_SURVEY_MODAL_KEY, ii as MODAL_CANCEL, oi as MODAL_CONFIRM, t as AutoSaveState, ua as EditorEnabledFeaturesKey, wa as CODE_WORKFLOW_BUILDER_EXPERIMENT } from "./constants-Bwbcx8Ih.js";
import { c as THREE_DAYS_IN_MILLIS, o as SEVEN_DAYS_IN_MILLIS, s as SIX_MONTHS_IN_MILLIS, t as DEBOUNCE_TIME } from "./durations-D9OTn4t5.js";
import { a as BUILDER_ENABLED_VIEWS, r as useChatPanelStateStore, t as useCodeDiff } from "./useCodeDiff-CL2-N3by.js";
import { t as getDebounceTime } from "./useDebounce-BRzd7gd1.js";
import { _ as tryToParseNumber, h as stringSizeInBytes } from "./typesUtils-BoKZFaOH.js";
import { a as getNewWorkflow, t as useWorkflowsListStore } from "./workflowsList.store-DR9pamv2.js";
...
//#region ../@n8n/design-system/src/types/assistant.ts
...
var useFocusedNodesStore = defineStore(STORES.FOCUSED_NODES, () => {
	...
	const isFeatureEnabled = computed(() => {
		return settingsStore.isCloudDeployment && posthogStore.isVariantEnabled(FOCUSED_NODES_EXPERIMENT.name, FOCUSED_NODES_EXPERIMENT.variant);
	});
	...
	watch(() => ndvStore.value.activeNode, (node) => {
...
async function createBuilderPayload(text, id, options) {
	...
	const featureFlags = {
		pinData: posthogStore.getVariant(CODE_WORKFLOW_BUILDER_EXPERIMENT.name) === CODE_WORKFLOW_BUILDER_EXPERIMENT.codePinData,
...
function useBuilderTodos() {
	...
	*/
	function nodeHasPinnedData(nodeName, visited = /* @__PURE__ */ new Set()) {
		if (visited.has(nodeName)) return false;
		visited.add(nodeName);
		if (workflowDocumentStore.value.pinnedDataByNodeName?.[nodeName]?.length) return true;
		const outgoingConnections = workflowDocumentStore.value.outgoingConnectionsByNodeName(nodeName);
		for (const connectionType of Object.keys(outgoingConnections)) {
			if (connectionType === "main") continue;
			const connections = outgoingConnections[connectionType];
...
var useWorkflowHistoryStore = defineStore("workflowHistory", () => {
	...
	const cloneIntoNewWorkflow = async (workflowId, workflowVersionId, data) => {
		...
		const { connections, nodes, nodeGroups } = workflowVersion;
		const { name } = workflow;
		const newWorkflow = await getNewWorkflow(rootStore.restApiContext, { name: `${name} (${data.formattedCreatedAt})` });
	...
	const restoreWorkflow = async (workflowId, workflowVersionId) => {
		const { connections, nodes, nodeGroups } = await getWorkflowVersion$1(workflowId, workflowVersionId);
		const updateData = {
			connections,
			nodes,
			nodeGroups: nodeGroups ?? []
	...
	const getPublishTimeline$1 = async (workflowId) => await getPublishTimeline(rootStore.restApiContext, workflowId);
	const getVersionFirstAdoptionDate = async (version) => await getFirstAdoptionDate(rootStore.restApiContext, version);
	return {
		...
		getPublishTimeline: getPublishTimeline$1,
...
function useEditorContext() {
	...
	const enabledInStore = (feature) => {
		switch (feature) {
			...
			case "instanceAi": return settings.isModuleActive("instance-ai") && settings.moduleSettings["instance-ai"]?.enabled !== false && (settings.moduleSettings["instance-ai"]?.setupCompleted === true || hasPermission(["rbac"], { rbac: { scope: "instanceAi:manage" } })) && hasPermission(["rbac"], { rbac: { scope: "instanceAi:message" } });
		}
	};
...
//#endregion
//#region src/app/composables/useInvalidNodeGroupCleanup.ts
...
*/
function useInvalidNodeGroupCleanup() {
	const nodeTypesStore = useNodeTypesStore();
	const rootStore = useRootStore();
	const toast = useToast();
	const telemetry = useTelemetry();
	const i18n = useI18n();
	function isGroupValid(store, group, allGroups) {
	...
	function showGroupsRemovedToast(removedGroups) {
		const groups = `<ul style="list-style-position: inside">${removedGroups.map((group) => `<li>${(0, import_lib.escapeHtml)(group.name)}</li>`).join("")}</ul>`;
		toast.showMessage({
			title: i18n.baseText("canvas.nodeGroup.removedOnSave.title"),
			message: i18n.baseText("canvas.nodeGroup.removedOnSave.message", { interpolate: { groups } }),
			type: "warning"
		});
	}
	/**
	...
	*/
	function removeInvalidNodeGroups(store) {
		...
		telemetry.track("Auto-ungrouped invalid node groups", { groups_affected: invalidGroups.length });
		showGroupsRemovedToast(invalidGroups);
		return invalidGroups;
	}
	return { removeInvalidNodeGroups };
}
//#endregion
...
function getErrorMessage(error) {
	...
	if (error && typeof error === "object" && "message" in error) {
		const { message } = error;
		if (message !== void 0) return String(message);
	}
	return String(error);
}
function getHttpStatusCode(error) {
	if (error instanceof ResponseError) return error.httpStatusCode;
	if (!error || typeof error !== "object") return;
	const { httpStatusCode, errorCode, response } = error;
	if (typeof httpStatusCode === "number") return httpStatusCode;
	if (response && typeof response === "object") {
		const { status } = response;
		if (typeof status === "number") return status;
	}
	if (typeof errorCode === "number" && errorCode >= 400 && errorCode < 600) return errorCode;
}
function shouldRetryAutoSaveFailure(error) {
...
function useWorkflowSaving({ router, onSaved, ownsAutoSave = false }) {
	const uiStore = useUIStore();
	...
	function showSaveErrorToast(errorMessage, retryDelay) {
		toast.showMessage({
			title: i18n.baseText("workflowHelpers.showMessage.title"),
			message: retryDelay === void 0 ? errorMessage : i18n.baseText("generic.autosave.retrying", { interpolate: {
				error: errorMessage,
				retryIn: `${Math.ceil(retryDelay / 1e3)}s`
			} }),
			type: "error",
			...retryDelay === void 0 ? {} : { duration: retryDelay }
		});
	}
	function scheduleAutoSaveRetry(retryDelay) {
	...
	function handleAutoSaveFailure(error, errorMessage) {
	...
	async function handleConflictSaveFailure({ error, errorMessage, currentWorkflow, id, redirect, autosaved }) {
		...
		if (autosaved) return handleAutoSaveFailure(error, errorMessage);
		return false;
	}
	const editorContext = getCurrentInstance() ? useEditorContext() : void 0;
	const canAutoSave = computed(() => ownsAutoSave && editorContext?.readOnly.value !== true);
	const currentWorkflowDocumentStore = computed(() => useWorkflowDocumentStore(createWorkflowDocumentId(workflowId.value)));
	const canArmAutoSave = computed(() => {
		if (!canAutoSave.value) return false;
		if (!settingsStore.isAutosaveEnabled) return false;
		if (!backendConnectionStore.isOnline) return false;
		if (!currentWorkflowDocumentStore.value.hydrated) return false;
		return true;
	});
	const canScheduleAutoSave = computed(() => {
		if (!canArmAutoSave.value) return false;
		if (saveStore.pendingSave) return false;
		if (saveStore.isRetrying) return false;
		return true;
	});
	async function promptSaveUnsavedWorkflowChanges(next, { confirm = async () => true, cancel = async () => {} } = {}) {
		...
		switch (await message.confirm(i18n.baseText("generic.unsavedWork.confirmMessage.message"), {
			...
			case MODAL_CANCEL:
				...
				return;
			case MODAL_CLOSE:
				if (workflowsStore.isWorkflowSaved[workflowId.value]) stayOnCurrentWorkflow(next);
				return;
		}
	}
	...
	async function saveCurrentWorkflow({ id } = {}, redirect = true, forceSave = false, autosaved = false) {
		...
		const savePromise = (async () => {
			...
			try {
				...
				console.error(error);
				if (isExistingWorkflowSave && getHttpStatusCode(error) === 409) return await handleConflictSaveFailure({
					error,
					errorMessage,
					currentWorkflow,
					id,
	...
	async function saveAsNewWorkflow({ name, tags, resetWebhookUrls, resetNodeIds, openInNewWindow, parentFolderId, uiContext, requestNewId, data, autosaved } = {}, redirect = true) {
		...
		try {
			const currentDocumentStore = useWorkflowDocumentStore(createWorkflowDocumentId(workflowId.value));
			if (!data) removeInvalidNodeGroups(currentDocumentStore);
			const dirtyCountBeforeSave = uiStore.dirtyStateSetCount;
			const workflowDataRequest = data || currentDocumentStore.serialize();
			if (!data && currentDocumentStore.description) workflowDataRequest.description = currentDocumentStore.description;
			const changedNodes = {};
			...
			if (resetNodeIds) {
				...
				if (workflowDataRequest.nodeGroups?.length) workflowDataRequest.nodeGroups = workflowDataRequest.nodeGroups.map((group) => ({
					...group,
			...
			if (resetWebhookUrls) workflowDataRequest.nodes = workflowDataRequest.nodes.map((node) => {
				if (node.webhookId) {
					...
					if ("path" in node.parameters && !isExpression(node.parameters.path)) node.parameters.path = newId;
			...
			workflowDocumentStore.setUpdatedAt(workflowData.updatedAt);
			if (workflowData.settings) workflowDocumentStore.setSettings(workflowData.settings);
			...
			if (autosaved && createRequestFailed) throw e;
			if (autosaved) {
				console.error(e);
				return null;
			}
	...
	const scheduleAutoSave = () => {
		if (!canScheduleAutoSave.value) return;
		saveStore.setAutoSaveState(AutoSaveState.Scheduled);
		autoSaveWorkflowDebounced();
	};
	...
	if (ownsAutoSave) watch(canArmAutoSave, (allowed, wasAllowed) => {
		if (allowed && !wasAllowed && uiStore.stateIsDirty) scheduleAutoSave();
	});
	return {
...
var useBuilderStore = defineStore(STORES.BUILDER, () => {
	...
	const workflowExecutionStateStore = computed(() => useWorkflowExecutionStateStore(createWorkflowDocumentId(routeWorkflowId.value)));
	...
	const { applyCodeDiff, undoCodeDiff } = useCodeDiff({
		chatMessages,
		getTargetNodeName: (msg) => msg.nodeName ?? ndvStore.value.activeNodeName ?? focusedNodesStore.confirmedNodes[0]?.nodeName ?? "",
		getSessionId: (msg) => {
	...
	*/
	...
	async function stopStreaming(payload) {
		...
		const postModVersion = hasWorkflowUpdate ? await savePostModificationVersion() : void 0;
	...
	*/
	async function savePostModificationVersion() {
		try {
			if (!await workflowSaver.saveCurrentWorkflow({}, false, true)) return void 0;
			const versionId = workflowDocumentStore.value.versionId;
			if (!versionId) return void 0;
			return await verifyVersionExists(versionId);
		} catch {
	...
	*/
		...
		try {
			...
			if (sessions.length > 0) {
				...
				if (posthogStore.getVariant(CODE_WORKFLOW_BUILDER_EXPERIMENT.name) === CODE_WORKFLOW_BUILDER_EXPERIMENT.codePinData) {
					const pinData = useWorkflowDocumentStore(createWorkflowDocumentId(routeWorkflowId.value)).getPinDataSnapshot();
	...
	function applyGeneratedPinData() {
		...
		const workflowDocumentStore = useWorkflowDocumentStore(createWorkflowDocumentId(routeWorkflowId.value));
	...
	*/
	...
	watch(() => routeWorkflowId.value, (newWorkflowId, oldWorkflowId) => {
	...
	watch([() => routeWorkflowId.value, () => workflowDocumentStore.value.allNodes.length], ([, nodesCount]) => {
...
//#endregion
export { isWorkflowUpdatedMessage$1 as C, isToolMessage$1 as S, isPlanModeUserAnswersMessage as _, useWorkflowSaveStore as a, isTaskAbortedMessage as b, useWorkflowHistoryStore as c, isPlaceholderValue as d, useFocusedNodesStore as f, isPlanModeQuestionsMessage as g, isPlanModePlanMessage as h, useBackendConnectionStore as i, extractPlaceholderLabels as l, isCollapsedGroupMessage as m, useBrowserNotifications as n, useEditorContext as o, createCollapsedGroupMessage as p, useWorkflowSaving as r, useNpsSurveyStore as s, useBuilderStore as t, findPlaceholderDetails as u, isVersionCardMessage as v, isThinkingGroupMessage as x, isWebFetchApprovalCustomMessage as y };
