...
import { bt as I18nT, yt as useI18n } from "./_MapCache-BRSIFZ-8.js";
import { Fi as N8nIconButton_default, Gi as N8nButton_default, Hi as N8nHeading_default, Ki as N8nIcon_default, O as N8nIconPicker_default, Pi as Input_default, Ui as N8nCallout_default, Wi as N8nText_default, ji as N8nTooltip_default, lt as N8nInputLabel_default } from "./src-CKCAtrpy.js";
import { t as __plugin_vue_export_helper_default } from "./_plugin-vue_export-helper-DltO58Gh.js";
import "./sanitize-html-JHjOJhXQ.js";
import { Tr as useToast, Vr as usePostHog, jr as useMessage, js as useSettingsStore, r as useUIStore, t as useUsersStore } from "./users.store-D86YL4LK.js";
import { Vc as CHAT_HUB_SEMANTIC_SEARCH_EXPERIMENT, _i as emptyChatModelsResponse } from "./constants-bR1gDMyV.js";
import "./merge-CIGhcbPZ.js";
import { r as useRootStore, v as assert } from "./_baseOrderBy-Bnq6lF9G.js";
...
import { c as CHAT_SETTINGS_VIEW, y as providerDisplayNames } from "./constants-CrjCGXcB.js";
import { E as fetchChatModelsApi, g as isLlmProviderModel, t as useChatStore, y as personalAgentDefaultIcon } from "./chat.store--01BHvRB.js";
import "./ChatAgentAvatar-rv19uatu.js";
...
var AgentEditorModalFileRow_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		const warningTooltip = computed(() => {
			if (props.item.isNew) return void 0;
			if (!props.semanticSearchReady) return i18n.baseText("chatHub.agent.editor.files.semanticSearchNotReady.tooltip");
			if (props.item.embeddingProvider && props.item.embeddingProvider !== props.currentEmbeddingProvider) return i18n.baseText("chatHub.agent.editor.files.embeddingMismatch.tooltip", { interpolate: {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", { class: normalizeClass(_ctx.$style.fileRow) }, [
				createBaseVNode("span", { class: normalizeClass(_ctx.$style.fileName) }, toDisplayString(__props.item.name), 3),
				createBaseVNode("div", { class: normalizeClass(_ctx.$style.indexedCell) }, [warningTooltip.value ? (openBlock(), createBlock(unref(N8nTooltip_default), {
					...
					default: withCtx(() => [createTextVNode(toDisplayString(__props.item.isNew ? __props.item.mimeType : unref(i18n).baseText("chatHub.agent.editor.files.indexed")), 1)]),
					_: 1
				}))], 2),
...
var AgentEditorModalFileRow_vue_vue_type_style_index_0_lang_module_default = {
	fileRow: "_fileRow_1lsy2_125",
	fileName: "_fileName_1lsy2_137",
	indexedCell: "_indexedCell_1lsy2_147",
	iconWarning: "_iconWarning_1lsy2_154"
};
...
var AgentEditorModal_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const allFiles = computed(() => [...savedFiles.value.map((file, index) => ({
			id: `saved-${index}`,
			name: file.fileName,
		...
		async function onSave() {
			...
			try {
				...
				if (isEditMode.value && props.data.agentId) {
					await chatStore.updateCustomAgent(props.data.agentId, payload, newFiles.value, removedFileKnowledgeIds.value, props.data.credentials);
					toast.showMessage({
						title: i18n.baseText("chatHub.agent.editor.success.update"),
						type: "success"
					});
				} else {
					const agent = await chatStore.createCustomAgent(payload, newFiles.value, props.data.credentials);
		...
		function removeExistingFile(index) {
			...
			if (file) removedFileKnowledgeIds.value = [...removedFileKnowledgeIds.value, file.id];
		...
		function removeFile(row$2) {
			if (row$2.isNew) removeNewFile(row$2.index);
			else removeExistingFile(row$2.index);
		}
