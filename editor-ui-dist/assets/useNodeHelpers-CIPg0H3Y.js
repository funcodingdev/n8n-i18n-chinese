...
import { t as useLoadingService } from "./useLoadingService-Ca2Jqw5H.js";
import { Pi as isString, Yt as injectWorkflowExecutionStateStore, a as injectWorkflowDocumentStore, ct as useCredentialsStore, ir as getNodeSubtitle, l as assignNodeId, lr as hasProxyAuth, v as useNodeTypesStore } from "./workflowDocument.store-CJmrbFRO.js";
...
import { Ac as EnterpriseEditionFeature, Au as getNodeInputs, Gu as makeNodeName, Ku as mergeIssues, Mu as getNodeParameters, Nu as getNodeParametersIssues, Ou as displayParameterPath, Yu as nodeIssuesToString, ku as getConnectionTypes, ld as NodeConnectionTypes, wl as classifyTriggerIdentity } from "./constants-BzyjfRY2.js";
import { n as hasPermission, p as useTelemetry } from "./users.store-CF55TUDI.js";
import { A as isObject } from "./workflowsList.store-DY3toAy4.js";
	...
	*/
	...
	function getBlockingTrigger() {
		...
		const trigger = triggers.find((t) => {
			const { providesN8nIdentity, providesExternalIdentity } = classifyTriggerIdentity(t.type, t.parameters);
	...
	function getTriggerDisplayName(trigger) {
		const displayName = nodeTypesStore.getNodeType(trigger.type, trigger.typeVersion)?.displayName ?? trigger.name;
	...
	function collectPrivateCredentialIssues(node, foundIssues) {
		...
		for (const [credTypeName, details] of Object.entries(node.credentials ?? {})) {
			...
			if (blockingTrigger) {
				const messageKey = blockingTrigger.isSystemResolver ? "nodeIssues.credentials.privateRequiresManualTrigger" : "nodeIssues.credentials.privateRequiresIdentityExtractor";
				foundIssues[credTypeName] = [i18n.baseText(messageKey, { interpolate: { triggerName: getTriggerDisplayName(blockingTrigger.trigger) } })];
			}
		}
	}
