...
import { Nt as useCredentialsStore, Pi as isValidNodeConnectionType, St as isTutorialTemplateId, _ as useWorkflowExecutionStateStore, b as injectWorkflowDocumentStore, f as openFormPopupWindow, h as parse, k as useNodeHelpers, l as getExecutionErrorMessage, mi as useExecutionDataStore, on as useWorkflowHelpers, pi as createExecutionDataId, q as useNodeTypesStore, r as clearPopupWindowState, rr as getTriggerNodeServiceName, t as useWorkflowsStore, u as getExecutionErrorToastConfiguration, v as createWorkflowDocumentId, vt as SampleTemplates, x as useWorkflowDocumentStore } from "./workflows.store-DZfkxcrn.js";
import { t as useToast } from "./useToast-ioerWMiY.js";
import { Os as generateNodesGraph, _c as isCommunityPackageName, bs as createRunExecutionData, il as TRIMMED_TASK_DATA_CONNECTIONS_KEY, qc as EVALUATION_TRIGGER_NODE_TYPE, ta as WORKFLOW_SETTINGS_MODAL_KEY } from "./constants-CLQJMg7Y.js";
import { T as defineStore, t as useRootStore } from "./useRootStore-BXKn7r8-.js";
import { at as sendGetWorkerStatus, t as useSettingsStore } from "./settings.store-HAN34Lkc.js";
...
import { r as useWorkflowSaving, t as useBuilderStore } from "./builder.store-CL34arwP.js";
import { i as globalLinkActionsEventBus, o as codeNodeEditorEventBus } from "./event-bus-H12DVska.js";
...
import { t as useActivationError } from "./useActivationError-8CrYlkM_.js";
import { t as useReadyToRunStore } from "./readyToRun.store-BuIhLOxY.js";
...
async function executionFinished({ data }, options) {
	...
	const belongsToThisDocument = activeExecutionId === data.executionId || activeExecutionId === null && data.workflowId === workflowExecutionStateStore.workflowId;
	...
	*/
	...
	else if (execution.status === "error" || execution.status === "canceled") handleExecutionFinishedWithErrorOrCanceled(execution, runExecutionData, documentId);
	else handleExecutionFinishedWithSuccessOrOther(documentId, execution.status, successToastAlreadyShown);
...
*/
function handleExecutionFinishedWithErrorOrCanceled(execution, runExecutionData, documentId) {
...
*/
function handleExecutionFinishedSuccessfully(workflowName, message, documentId) {
	const toast = useToast();
...
* Handle the case when the workflow execution finished successfully.
*/
function handleExecutionFinishedWithSuccessOrOther(documentId, executionStatus, successToastAlreadyShown) {
	const workflowsStore = useWorkflowsStore();
	const toast = useToast();
	const i18n = useI18n();
	...
	if (workflowExecution?.executedNode) {
		...
		else if (!successToastAlreadyShown) handleExecutionFinishedSuccessfully(workflowName, i18n.baseText("pushConnection.nodeExecutedSuccessfully"), documentId);
	} else if (!successToastAlreadyShown) handleExecutionFinishedSuccessfully(workflowName, i18n.baseText("pushConnection.workflowExecutedSuccessfully"), documentId);
...
async function executionRecovered({ data }, options) {
	...
	else handleExecutionFinishedWithSuccessOrOther(documentId, execution.status, false);
	setRunExecutionData(execution, runExecutionData, documentId);
}
//#endregion
