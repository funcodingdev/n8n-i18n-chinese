...
import { v as require_get } from "./src-DXhOkTfb.js";
import { $r as RemoveConnectionCommand, Bt as useWorkflowSaving, Dt as injectWorkflowState, Et as useWorkflowHelpers, Is as useSettingsStore, Jr as AddNodeCommand, Kr as useHistoryStore, Ln as useToast, Wr as CanvasNodeDirtiness, Yr as BulkCommand, Zr as EnableNodeToggleCommand, ca as createWorkflowDocumentId, da as useWorkflowDocumentStore, ei as RemoveNodeCommand, gr as useTelemetry, h as displayForm, pr as useNodeHelpers, qr as AddConnectionCommand, r as useUIStore, s as useWorkflowsStore, wa as isEmpty, zn as useExternalHooks, zt as useDocumentTitle } from "./users.store-bYJP9NEL.js";
import { Gt as CHAT_TRIGGER_NODE_TYPE, Ht as CHAT_HITL_TOOL_NODE_TYPE, Ii as createResultError, Li as createResultOk, N as IN_PROGRESS_EXECUTION_ID, Ut as CHAT_NODE_TYPE, Wt as CHAT_TOOL_NODE_TYPE, ca as createRunExecutionData, go as NodeConnectionTypes, ma as generateNodesGraph, nr as RESPOND_TO_WEBHOOK_NODE_TYPE, wo as BINARY_MODE_COMBINED } from "./constants-neq36s1h.js";
import { j as defineStore, m as request, r as useRootStore } from "./_baseOrderBy-2EL3KkTw.js";
import { t as useExecutionsStore } from "./executions.store-CXfJa1PO.js";
import { S as useChat, l as chatEventBus, t as useCanvasOperations } from "./useCanvasOperations-Cc3fbdBt.js";
...
function useRunWorkflow(useRunWorkflowOpts) {
	...
	async function runWorkflow(options) {
		...
		try {
			...
			if (options.destinationNode && (workflowsStore.checkIfNodeHasChatParent(options.destinationNode.nodeName) || workflowsStore.checkIfToolNodeHasChatParent(options.destinationNode.nodeName) || destinationNodeType === "@n8n/n8n-nodes-langchain.chatTrigger") && options.source !== "RunData.ManualChatMessage" && options.source !== "RunData.ManualChatTrigger") {
	...
	async function runEntireWorkflow(source, triggerNode) {
		...
		if (!resolvedTriggerNode) {
			const triggers = Object.values(workflowObject.value.nodes).filter((node) => !node.disabled && node.type.toLowerCase().includes("trigger"));
