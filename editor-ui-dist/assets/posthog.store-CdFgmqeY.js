...
import { O as useStorage$1 } from "./dist-CYNfKCbA.js";
...
import { Ii as LOCAL_STORAGE_EXPERIMENT_OVERRIDES, ka as EXPERIMENTS_TO_TRACK } from "./constants-Bwbcx8Ih.js";
...
var usePostHog = defineStore("posthog", () => {
	...
	const featureFlagPayloads = ref(null);
	...
	const getFeatureFlagPayload = (flag) => {
	...
	*/
	...
	if (!window.featureFlags) {
		...
		if (cachedOverrides) try {
			...
			if (typeof parsedOverrides === "object") overrides.value = Object.fromEntries(Object.entries(parsedOverrides).map(([name, override]) => [name, typeof override === "object" ? override : { value: override }]));
		} catch (e) {
			console.log("Could not override experiment", e);
		}
		window.featureFlags = {
			override: (name, value, payload) => {
				overrides.value[name] = {
					value,
					...payload !== void 0 && payload !== null ? { payload } : {}
				};
				try {
	...
	const init = (evaluatedFeatureFlags, evaluatedFeatureFlagPayloads) => {
		...
		if (evaluatedFeatureFlags && Object.keys(evaluatedFeatureFlags).length) {
			options.bootstrap = {
				...
				...evaluatedFeatureFlagPayloads && { featureFlagPayloads: evaluatedFeatureFlagPayloads }
			};
		...
		if (evaluatedFeatureFlags && Object.keys(evaluatedFeatureFlags).length) {
			...
			window.posthog?.onFeatureFlags?.((_, map) => {
				...
				for (const key of Object.keys(map)) {
					...
					if (payload !== void 0 && payload !== null) payloads[key] = payload;
				}
