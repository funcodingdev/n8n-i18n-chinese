...
import { P as getCurrentInstance, Pt as reactive, S as computed, Ut as toValue } from "./vue.runtime.esm-bundler-DYHsQBZB.js";
import { s as useI18n } from "./src-C3h3FWXj.js";
import { V as until, b as useLocalStorage } from "./dist-CYNfKCbA.js";
import { o as useEditorContext, r as useWorkflowSaving } from "./builder.store-sZXvAM9C.js";
import { t as useMessage } from "./useMessage-CMftD8l5.js";
import { A as useWorkflowDocumentStore, O as injectWorkflowDocumentStore, Zi as CanvasNodeDirtiness, en as useWorkflowHelpers, i as displayForm, t as useWorkflowsStore, y as useWorkflowExecutionStateStore } from "./workflows.store-CQp5NuTc.js";
import { Io as require_get, Ir as createRunExecutionData, Mn as createResultError, Nn as createResultOk, Si as generateNodesGraph, la as NodeConnectionTypes } from "./src-BrbN-vPn.js";
import { nr as request, t as useRootStore, ur as defineStore } from "./useRootStore-B-A2Ktsc.js";
...
import { M as IN_PROGRESS_EXECUTION_ID, Pt as CHAT_TRIGGER_NODE_TYPE } from "./constants-Bwbcx8Ih.js";
...
import { c as EnableNodeToggleCommand, d as RemoveNodeCommand, i as AddNodeCommand, n as useHistoryStore, o as BulkCommand, r as AddConnectionCommand, t as useNodeHelpers, u as RemoveConnectionCommand } from "./useNodeHelpers-B7gC71EA.js";
...
import { M as useChat, t as useCanvasOperations, y as chatEventBus } from "./useCanvasOperations-Bb1O1ttk.js";
import { t as retry } from "./retry-BfdErGBy.js";
...
*/
function useNodeDirtiness(workflowDocumentId) {
	const historyStore = useHistoryStore();
	const workflowDocumentStore = computed(() => useWorkflowDocumentStore(toValue(workflowDocumentId)));
	const executionStateStore = computed(() => useWorkflowExecutionStateStore(toValue(workflowDocumentId)));
...
function useRunWorkflow(useRunWorkflowOpts) {
	...
	const workflowDocumentStore = useRunWorkflowOpts.workflowDocumentStore ?? injectWorkflowDocumentStore();
	...
	async function runWorkflowApi(runData) {
		...
		const workflowExecutionIdIsNew = workflowExecutionState.value.previousExecutionId !== response.executionId;
		const workflowExecutionIdIsPending = workflowExecutionState.value.activeExecutionId === null;
		if (response.executionId && workflowExecutionIdIsNew && workflowExecutionIdIsPending) workflowExecutionState.value.setActiveExecutionId(response.executionId);
		if (response.waitingForWebhook === true) workflowExecutionState.value.setExecutionWaitingForWebhook(true);
		return response;
	}
	async function runWorkflow(options) {
		if (workflowExecutionState.value.activeExecutionId) return;
		...
		try {
			...
			if (!isNewWorkflow && uiStore.stateIsDirty && !settingsStore.isAutosaveEnabled) {
				if (await message.confirm(i18n.baseText("workflowRun.saveBeforeRun.message"), {
				...
				if (!await workflowSaving.saveCurrentWorkflow({ id: workflowDocumentStore.value.workflowId })) return;
			} else if (isNewWorkflow || uiStore.stateIsDirty && settingsStore.isAutosaveEnabled) await workflowSaving.saveCurrentWorkflow({ id: workflowDocumentStore.value.workflowId });
			...
			if (options.destinationNode && (workflowDocumentStore.value.checkIfNodeHasChatParent(options.destinationNode.nodeName) || workflowDocumentStore.value.checkIfToolNodeHasChatParent(options.destinationNode.nodeName) || destinationNodeType === "@n8n/n8n-nodes-langchain.chatTrigger") && options.source !== "RunData.ManualChatMessage" && options.source !== "RunData.ManualChatTrigger") {
				...
				if (startNode && startNode.type === "@n8n/n8n-nodes-langchain.chatTrigger") {
					...
					if (!chatHasInputData && !chatHasPinData) {
						workflowExecutionState.value.setChatPartialExecutionDestinationNode(options.destinationNode.nodeName);
						startChat();
						return;
					}
				}
			...
			if (editorContext?.executionErrorToasts.value !== false) toast.showError(error, i18n.baseText("workflowRun.showError.title"));
			return;
		}
	}
	...
	async function stopCurrentExecution() {
		...
		let stopData;
		if (executionId === null) executionId = await until(() => workflowExecutionState.value.activeExecutionId).toMatch((id) => id !== null, { timeout: 1e4 });
		...
		try {
			...
			if (!await retry(async () => {
				...
				return false;
			}, 250, 20)) workflowExecutionState.value.markExecutionAsStopped(stopData);
		}
	}
