import { $ as openBlock, Gt as unref, It as ref, N as defineComponent, S as computed, Ut as toValue, X as onMounted, bt as withCtx, gt as watch, j as createVNode, tt as provide, w as createBlock } from "./vue.runtime.esm-bundler-DYHsQBZB.js";
import { s as useI18n } from "./src-C3h3FWXj.js";
import { l as useRouter } from "./vue-router-D2dKRIiV.js";
import { Ct as useAiGatewayStore, H as useNodeTypesStore, K as filterAndSearchNodes, St as stripToolSuffix, bn as useProjectsStore, nt as isNodePreviewKey, ot as removePreviewToken, t as useWorkflowsStore, tt as isAiGatewayEligibleNode, xn as useSourceControlStore } from "./workflows.store-CQp5NuTc.js";
import { G as INCOMPATIBLE_WORKFLOW_TOOL_BODY_NODE_TYPES, Nt as getResourcePermissions, Yi as resolveSupportedCredentialActivation, io as v4, la as NodeConnectionTypes, va as isCommunityPackageName } from "./src-BrbN-vPn.js";
...
import { Or as AGENT_TOOL_CONFIG_MODAL_KEY, n as DEFAULT_NEW_WORKFLOW_NAME } from "./constants-Bwbcx8Ih.js";
import { a as TELEMETRY_EVENT } from "./src-BmGqsjIb.js";
import { s as getWorkflow } from "./workflowsList.store-DR9pamv2.js";
import { n as useUIStore } from "./ui.store-ic9znGOr.js";
import { n as getNodeIconSource } from "./nodeIcon-Bv1XEyIQ.js";
import { t as useInstallNode } from "./useInstallNode-CccmsCZf.js";
import { n as SAMPLE_SUBWORKFLOW_TRIGGER_ID, r as SAMPLE_SUBWORKFLOW_WORKFLOW } from "./samples-CMRT93fJ.js";
import { a as workflowToolTriggerLabel, c as toolRefToNode, d as workflowToNewToolRef, i as nodeTypeToNewMcpServer, n as mcpServerToNode, o as getExistingToolNames, s as nodeTypeToNewToolRef, t as isMcpRelatedNodeType } from "./useMcpServerAdapter-BFbM7B-M.js";
import { n as toolCategoryForNodeType, r as useAgentToolCatalog, t as hasInputs } from "./useAgentToolCatalog-DmVIy8hj.js";
import { a as TOOL_CONNECTION_CREDITS_LABEL_KEY, o as hasToolConnection } from "./ToolIcon-BbwTPlls.js";
import { t as ToolsConnectionModal_default } from "./ToolsConnectionModal-iVwNcZMR.js";
import { t as McpRegistrySuggestionFooter_default } from "./McpRegistrySuggestionFooter-CYkUghtV.js";
...
//#endregion
//#region src/features/agents/components/AgentToolsConnectionModalWrapper.vue?vue&type=script&setup=true&lang.ts
var N8N_CONNECT_ID_PREFIX = "n8n-connect:";
//#endregion
...
var AgentToolsConnectionModalWrapper_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		provide(TOOL_CONNECTION_CREDITS_LABEL_KEY, computed(() => aiGatewayStore.creditsLabelKey));
		...
		*/
		function addManagedNodeTool(nodeType) {
			toolTelemetry.trackAddStarted("node");
			const newRef = nodeTypeToNewToolRef(nodeType);
			const activation = resolveSupportedCredentialActivation(nodeType, {
				typeVersion: newRef.node.nodeTypeVersion,
				parameters: {}
			}, aiGatewayStore.isCredentialTypeSupported);
			if (activation) {
				newRef.node.nodeParameters = activation.parameters;
				newRef.node.credentials = { [activation.credentialType]: {
					id: null,
					name: "",
		...
		*/
		function n8nConnectNodeItem(nodeType) {
			return {
		...
		function availableWorkflowItem(workflow) {
			return {
				...
				warning: workflow.activeVersionId === null ? i18n.baseText("agents.tools.workflow.notPublished") : void 0,
				status: "none",
				credentials: []
			};
		}
		...
		function disabledWorkflowReasonText(reason) {
			...
			return i18n.baseText("agents.tools.workflow.disabled.noSupportedTrigger", { interpolate: { trigger: workflowToolTriggerLabel() } });
		}
		/**
		...
		*/
		...
		const n8nConnectItems = computed(() => availableToolTypes.value.filter((nodeType) => isAiGatewayEligibleNode(nodeType.name)).map(n8nConnectNodeItem));
		/**
		...
		*/
		...
		const items = computed(() => {
			const out = [];
			for (const item of n8nConnectItems.value) out.push(item);
		...
		function handleRowActivate(item) {
			...
			if (hasToolConnection(item.status)) {
				...
				if (item.id.startsWith("tool:")) {
					...
					const { ref } = entry;
					if (ref.type === "node") {
						const nodeType = [...availableToolTypes.value, ...communitySearchToolTypes.value].find((nt) => nt.name === ref.node.nodeType) ?? nodeTypesStore.getNodeType(ref.node.nodeType);
						if (nodeType) Object.values(ref.node.credentials ?? {}).some((credential) => "__aiGatewayManaged" in credential && credential.__aiGatewayManaged) ? addManagedNodeTool(nodeType) : handleAddTool(nodeType);
						return;
					}
			...
			if (item.kind === "node" && item.id.startsWith(N8N_CONNECT_ID_PREFIX)) {
				const nodeTypeName = item.id.slice(12);
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(ToolsConnectionModal_default, {
				...
				"suggestion-footer": withCtx(() => [createVNode(McpRegistrySuggestionFooter_default, {
					prompt: unref(i18n).baseText("agents.tools.suggestion.prompt"),
					action: unref(i18n).baseText("agents.tools.suggestion.action")
				}, null, 8, ["prompt", "action"])]),
				_: 1
			}, 8, [
				"open",
				"items",
