...
import { it as require_startCase } from "./workflowDocument.store-CJmrbFRO.js";
import { Ga as normalizeMetricScore, Ua as ONE_TO_FIVE_METRIC_KEYS, Wa as RESERVED_METRIC_KEYS } from "./constants-BzyjfRY2.js";
...
function buildScoreShapedMetricGroups(runs) {
	...
	return orderedKeys.filter((key) => runs.some((run) => run.metrics?.[key] !== void 0) && runs.every((run) => {
		...
		return value === void 0 || normalizeMetricScore(key, value) !== null;
	})).map((key) => ({
		key,
		values: runs.map((run) => {
			const value = run.metrics?.[key];
...
function averageNormalizedScore(metrics) {
	if (!metrics) return null;
	const values = Object.entries(metrics).map(([key, value]) => normalizeMetricScore(key, value)).filter((value) => value !== null);
...
//#endregion
export { normalizeMetricValue as C, indexOfMax as S, getDeltaTone as _, computeDurationMs as a, getOperationalMetricEntries as b, extractCaseAnswer as c, formatMetricLabel as d, formatMetricPercent as f, formatTokens as g, formatShortDateTime as h, computeDelta as i, formatDeltaPercent as l, formatMetricRawScoreSum as m, buildScoreShapedMetricGroups as n, deriveRunsStatus as o, formatMetricRawScore as p, casePassed as r, extractAnswerText as s, averageNormalizedScore as t, formatDuration as u, getMetricCategory as v, stringifyValue as w, getUserDefinedMetricNames as x, getMetricDescriptionKey as y };
