...
import { o as useEditorContext, r as useWorkflowSaving, t as useBuilderStore } from "./builder.store-BxrHsmhL.js";
import { A as useWorkflowDocumentStore, Ci as useExecutionDataStore, Ct as useAiGatewayStore, Et as useCredentialsStore, H as useNodeTypesStore, O as injectWorkflowDocumentStore, T as createWorkflowDocumentId, Ui as isValidNodeConnectionType, _t as isTutorialTemplateId, bi as createExecutionDataId, en as useWorkflowHelpers, g as parse, l as getExecutionErrorMessage, nr as getTriggerNodeServiceName, p as openFormPopupWindow, pt as SampleTemplates, r as clearPopupWindowState, t as useWorkflowsStore, u as getExecutionErrorToastConfiguration, y as useWorkflowExecutionStateStore } from "./workflows.store-C0TGOSWn.js";
import { Ci as isCommunityPackageName, Dr as generateNodesGraph, Yi as EVALUATION_TRIGGER_NODE_TYPE, Yn as createRunExecutionData, sa as TRIMMED_TASK_DATA_CONNECTIONS_KEY } from "./src-B2jBAc9z.js";
import { fr as defineStore, t as useRootStore, vt as sendGetWorkerStatus } from "./useRootStore-B4L7-X8D.js";
...
import { Si as WORKFLOW_SETTINGS_MODAL_KEY } from "./constants-DOw6b_w2.js";
import { t as useWorkflowsListStore } from "./workflowsList.store-5H3D_H0z.js";
import { a as has, n as useUIStore } from "./ui.store-CUIioIti.js";
...
import { i as globalLinkActionsEventBus, o as codeNodeEditorEventBus } from "./event-bus-DZt3eR0N.js";
...
import { t as useActivationError } from "./useActivationError-C3O3RJ6v.js";
import { t as createEventQueue } from "./create-event-queue-DarUe4mm.js";
import { t as useReadyToRunStore } from "./readyToRun.store-CZ548Vgq.js";
import { t as useAITemplatesStarterCollectionStore } from "./aiTemplatesStarterCollection.store-CxLQh_WZ.js";
//#region src/app/composables/usePushConnection/handlers/agentNodeProgress.ts
async function agentNodeProgress(event, { documentId }) {
...
*/
function refreshWalletAfterBilledRun(documentId) {
	if (!useSettingsStore().isAiGatewayEnabled) return;
	const aiGatewayStore = useAiGatewayStore();
	const { nodes } = useWorkflowDocumentStore(documentId).getSnapshot();
	if (nodes.some((node) => aiGatewayStore.hasGatewayManagedCredential(node))) aiGatewayStore.fetchWallet({ force: true });
}
/**
...
async function fetchExecutionData(executionId, documentId) {
	...
	try {
		...
		return {
			...
			startedAt: useWorkflowExecutionStateStore(documentId).activeExecution?.startedAt,
...
async function workflowActivated({ data }, { documentId }) {
	...
	if (workflowIsBeingViewed && activeVersionChanged) {
		...
		if (uiStore.stateIsDirty) workflowDocumentStore.setChecksum(updatedWorkflow.checksum);
		else await initializeWorkspace(updatedWorkflow);
	}
	if (workflowIsBeingViewed) {
		if (useSettingsStore().isWorkflowPublicationServiceEnabled) workflowDocumentStore.setPublicationStatus({
			status: "published",
			failures: []
		});
...
async function workflowDeactivated({ data }, { documentId }) {
	...
	const uiStore = useUIStore();
	if (workflowDocumentStore.workflowId === data.workflowId) {
		if (useSettingsStore().isWorkflowPublicationServiceEnabled) workflowDocumentStore.setPublicationStatus({ status: "idle" });
		const updatedWorkflow = await workflowsListStore.fetchWorkflow(data.workflowId);
		if (!updatedWorkflow.checksum) throw new Error("Failed to fetch workflow");
		if (uiStore.stateIsDirty) {
			workflowDocumentStore.setActiveState({
...
async function workflowPartiallyActivated({ data }, { documentId }) {
	...
	if (workflowDocumentStore.activeVersionId !== activeVersionId) {
		const updatedWorkflow = await workflowsListStore.fetchWorkflow(workflowId);
		if (!updatedWorkflow.checksum) throw new Error("Failed to fetch workflow");
		if (uiStore.stateIsDirty) workflowDocumentStore.setChecksum(updatedWorkflow.checksum);
		else await initializeWorkspace(updatedWorkflow);
...
function usePushConnection({ router }) {
	...
	*/
	async function processEvent(event) {
		...
		if (has(event.type)) return;
		switch (event.type) {
			case "agentNodeProgress": return await agentNodeProgress(event, options);
