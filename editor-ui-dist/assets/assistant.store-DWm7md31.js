...
import { pt as createEventBus, yi as useRoute } from "./src-wu1DeT-q.js";
import { It as useNDVStore, Nt as useTelemetry, Pa as CREDENTIAL_EDIT_MODAL_KEY, fa as codeNodeEditorEventBus, gr as useToast, hs as useSettingsStore, it as chatWithAssistant, jc as STORES, jr as v4_default, or as hasPermission, ot as replaceCode, r as useUIStore, rt as useCredentialsStore, s as useWorkflowsStore, st as useAIAssistantHelpers, t as useUsersStore } from "./users.store-3JSNDD0x.js";
import { $o as EDITABLE_CANVAS_VIEWS, za as deepCopy } from "./constants-BxuLnwsr.js";
import { A as defineStore, _ as assert, r as useRootStore } from "./_baseOrderBy-Bum33Aot.js";
...
const useAssistantStore = defineStore(STORES.ASSISTANT, () => {
	...
	async function getVisualContext(nodeInfo) {
		...
		const activeNodeForLLM = activeNode ? await assistantHelpers.processNodeForAssistant(activeNode, ["position", "parameters.notice"], { trimParameterValues: !allowSendingParameterValues.value }) : null;
		...
		return {
			...
			currentWorkflow: workflowDataStale.value ? await assistantHelpers.simplifyWorkflowForAssistant(workflowsStore.workflow, { trimParameterValues: !allowSendingParameterValues.value }) : void 0,
	...
	async function initSupportChat(userMessage, credentialType) {
		...
		const activeNode = workflowsStore.activeNode();
		const nodeInfo = assistantHelpers.getNodeInfoForAssistant(activeNode, { trimParameterValues: !allowSendingParameterValues.value });
	...
	async function initErrorHelper(context) {
		...
		const { authType, nodeInputData, schemas } = assistantHelpers.getNodeInfoForAssistant(context.node, { trimParameterValues: !allowSendingParameterValues.value });
		...
		const payload = {
			...
			user: { firstName: usersStore.currentUser?.firstName ?? "" },
			error: context.error,
			node: await assistantHelpers.processNodeForAssistant(context.node, ["position", "parameters.notice"], { trimParameterValues: !allowSendingParameterValues.value }),
			nodeInputData,
	...
	async function sendMessage(chatMessage) {
		...
		try {
			...
			const activeNode = workflowsStore.activeNode();
			const userContext = await getVisualContext(assistantHelpers.getNodeInfoForAssistant(activeNode, { trimParameterValues: !allowSendingParameterValues.value }));
			chatWithAssistant(rootStore.restApiContext, {
				payload: {
					id: v4_default(),
					role: "user",
