...
import { $ as openBlock, A as createTextVNode, C as createBaseVNode, Cn as toDisplayString, E as createElementBlock, Gt as unref, It as ref, N as defineComponent, S as computed, T as createCommentVNode, _ as Fragment, bt as withCtx, h as withModifiers, j as createVNode, rt as renderList, vn as normalizeClass, w as createBlock } from "./vue.runtime.esm-bundler-DYHsQBZB.js";
...
import { H as useDebounceFn, l as useAsyncState } from "./dist-CYNfKCbA.js";
...
import { t as SettingsLayout_default } from "./SettingsLayout-C-2LE__y.js";
...
import { ri as MIGRATE_WORKFLOW_MODAL_KEY } from "./constants-Bwbcx8Ih.js";
...
import { n as getReportForRule } from "./breaking-changes-16d2pzoR.js";
import { t as ResourceFiltersDropdown_default } from "./ResourceFiltersDropdown-gi90aVr1.js";
import { t as SeverityTag_default } from "./SeverityTag-BRcoXZFP.js";
...
var MigrationRuleDetail_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const tableHeaders = computed(() => {
			...
			if (state.value.migratable) headers.push({
				title: "",
				key: "actions",
				value: () => "",
				width: 40,
		...
		const nodeIssues = (issues) => issues.filter((issue) => issue.nodeId !== void 0);
		const migratedWorkflowIds = ref(/* @__PURE__ */ new Set());
		const migrateModalBus = createEventBus();
		migrateModalBus.on("migrated", ({ workflowId }) => {
			migratedWorkflowIds.value = new Set(migratedWorkflowIds.value).add(workflowId);
		});
		function openMigrateModal(workflow) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(unref(SettingsLayout_default), {
				...
				default: withCtx(() => [
					...
					(openBlock(), createBlock(unref(N8nDataTableServer_default), {
						key: String(unref(state).migratable),
						...
						[`item.issues`]: withCtx(({ item }) => [createBaseVNode("div", _hoisted_2, [(openBlock(true), createElementBlock(Fragment, null, renderList(nodeIssues(item.issues), (issue, index) => {
							return openBlock(), createElementBlock(Fragment, { key: issue.nodeId }, [index > 0 ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [createTextVNode(", ")], 64)) : createCommentVNode("", true), createVNode(unref(N8nLink_default), {
								theme: "text",
						...
						[`item.actions`]: withCtx(({ item }) => [migratedWorkflowIds.value.has(item.id) ? (openBlock(), createBlock(unref(N8nText_default), {
							key: 0,
							color: "text-light",
							size: "small"
						}, {
							default: withCtx(() => [createTextVNode(toDisplayString(unref(i18n).baseText("settings.migrationReport.detail.migrate.migrated")), 1)]),
							_: 1
						})) : (openBlock(), createBlock(unref(N8nButton_default), {
							key: 1,
							size: "small",
							label: unref(i18n).baseText("settings.migrationReport.detail.migrate.button"),
							"data-test-id": "migrate-workflow-button",
							onClick: withModifiers(($event) => openMigrateModal(item), ["stop"])
