...
import { At as isRef, Bt as toRef, Et as getCurrentScope, Ft as ref, L as hasInjectionContext, Mt as onScopeDispose, Nt as reactive, R as inject, S as computed, Tt as effectScope, Vt as toRefs, W as nextTick, gt as watch, jt as markRaw, kt as isReactive, zt as toRaw } from "./vue.runtime.esm-bundler-dg1EVmSK.js";
import { dt as isVue2, ft as set, m as require_isArray } from "./_MapCache-BRSIFZ-8.js";
import { C as require__arrayMap, E as require_isSymbol, y as require__baseGet } from "./src-CKCAtrpy.js";
import { Bi as require__baseForOwn, Qa as jsonParse, ca as setGlobalState, qo as ApplicationError, to as randomString, zi as require__baseIteratee } from "./constants-bR1gDMyV.js";
import { E as require__baseUnary, b as require_isArrayLike, o as require_identity } from "./merge-CIGhcbPZ.js";
var activePinia;
...
var AxiosError$1 = class AxiosError$1 extends Error {
	static from(error, code, config, request$1, response, customProps) {
		const axiosError = new AxiosError$1(error.message, code || error.code, config, request$1, response);
		axiosError.cause = error;
		axiosError.name = error.name;
		customProps && Object.assign(axiosError, customProps);
		return axiosError;
	}
	constructor(message, code, config, request$1, response) {
		super(message);
...
var CanceledError$1 = class extends AxiosError_default {
	constructor(message, config, request$1) {
		super(message == null ? "canceled" : message, AxiosError_default.ERR_CANCELED, config, request$1);
...
var factory = (env) => {
	...
	return async (config) => {
		...
		try {
			...
			let response = await (isRequestSupported ? _fetch(request$1, fetchOptions) : _fetch(url, resolvedOptions));
			...
			unsubscribe && unsubscribe();
			if (err && err.name === "TypeError" && /Load failed|fetch/i.test(err.message)) throw Object.assign(new AxiosError_default("Network Error", AxiosError_default.ERR_NETWORK, config, request$1, err && err.response), { cause: err.cause || err });
			throw AxiosError_default.from(err, err && err.code, config, request$1, err && err.response);
		}
	};
};
