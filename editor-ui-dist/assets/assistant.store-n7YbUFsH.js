...
import { At as useCodeDiff, Ns as useSettingsStore, Qa as CREDENTIAL_EDIT_MODAL_KEY, aa as createWorkflowDocumentId, at as useCredentialsStore, ca as useWorkflowDocumentStore, ct as useAIAssistantHelpers, dl as STORES, li as hasPermission, ot as chatWithAssistant, pr as useTelemetry, qn as useChatPanelStateStore, r as useUIStore, s as useWorkflowsStore, t as useUsersStore } from "./users.store-Qyayth1w.js";
import { Wo as v4_default, ps as EDITABLE_CANVAS_VIEWS } from "./constants-CbM6Kczw.js";
import { j as defineStore, r as useRootStore, v as assert } from "./_baseOrderBy-DWejZi4o.js";
var READABLE_TYPES = [
	"code-diff",
	"text",
	"block"
...
const useAssistantStore = defineStore(STORES.ASSISTANT, () => {
	...
	async function sendMessage(workflowId, chatMessage) {
		...
		try {
			...
			const activeNode = workflowsStore.activeNode();
			const userContext = await getVisualContext(assistantHelpers.getNodeInfoForAssistant(workflowId, activeNode, { excludeParameterValues: !allowSendingParameterValues.value }));
			if (streamingAbortController.value) streamingAbortController.value.abort();
			streamingAbortController.value = new AbortController();
			chatWithAssistant(rootStore.restApiContext, {
				payload: {
	...
	function trackUserOpenedAssistant({ source, task, has_existing_session }) {
		const canvasStatus = (workflowDocumentStore.value?.allNodes ?? []).length === 0 ? "empty" : "existing_workflow";
