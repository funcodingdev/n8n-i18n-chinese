...
import { $i as toMegaBytes, $t as createWorkflowDocumentId, Fr as pinDataToExecutionData, Jt as useTelemetry, Pr as getPinDataSize, Qi as stringSizeInBytes, ht as useDataSchema, nn as useWorkflowDocumentStore, r as useUIStore, s as useWorkflowsStore, tn as injectWorkflowDocumentStore, w as useNodeTypesStore } from "./users.store-Zd_8U4jS.js";
import { t as useToast } from "./useToast-DG5hVw1V.js";
import { Bn as PIN_DATA_NODE_TYPES_DENYLIST, Cs as NodeConnectionTypes, Lo as isSubNodeType, To as getNodeOutputs, ac as MAX_EXPECTED_REQUEST_SIZE, os as jsonParse, sc as MAX_WORKFLOW_SIZE, ss as jsonStringify } from "./constants-c-L9DE1I.js";
...
function usePinnedData(node, options = {}) {
	...
	const workflowDocumentStore = injectWorkflowDocumentStore() ?? shallowRef(useWorkflowDocumentStore(createWorkflowDocumentId(workflowsStore.workflowId)));
	...
	function setData(data, source) {
		...
		if (workflowDocumentStore.value) {
			...
			if ((workflowDocumentStore.value.pinData[nodeName] ?? []).length > 0) workflowDocumentStore.value.touchPinnedDataLastUpdatedAt(nodeName);
