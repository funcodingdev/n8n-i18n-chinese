...
import { M as useLocalStorage, S as useClipboard, bt as useI18n } from "./_MapCache-BLQlsGfT.js";
import { Ni as require_markdown_it_link_attributes, _t as useRouter, gt as useRoute } from "./src-C3dDmkfF.js";
import { t as __plugin_vue_export_helper_default } from "./_plugin-vue_export-helper-DltO58Gh.js";
import { $t as useNodeHelpers, An as CONFIGURATION_NODE_SIZE, As as useSettingsStore, Ci as MoveNodeCommand, Di as ReplaceNodeParametersCommand, Dn as useTemplatesStore, Dr as useExternalHooks, Ei as RenameNodeCommand, Et as useWorkflowHelpers, Ft as useProjectsStore, Hn as getNewNodePosition, Hr as usePostHog, In as doRectsOverlap, Ln as generateOffsets, Mn as DEFAULT_VIEWPORT_BOUNDARIES, Nn as GRID_SIZE, On as CONFIGURABLE_NODE_SIZE, Or as useCanvasStore, Pn as PUSH_NODES_OFFSET, Sn as useFocusPanelStore, Ta as tryToParseNumber, Ti as RemoveNodeCommand, Tr as useToast, Wn as getNodesGroupSize, _i as useHistoryStore, aa as useWorkflowDocumentStore, an as useWorkflowState, at as useCredentialsStore, b as getExecutionErrorToastConfiguration, ba as isPresent, ca as pinDataToExecutionData, d as isChatNode, en as useTelemetry, gi as CanvasNodeRenderType, in as injectWorkflowState, jn as DEFAULT_NODE_SIZE, kt as useTagsStore, lt as useDataSchema, mi as CanvasConnectionMode, na as createWorkflowDocumentId, r as useUIStore, rn as useNDVStore, s as useWorkflowsStore, ui as isValidNodeConnectionType, vi as AddConnectionCommand, w as useNodeTypesStore, wi as RemoveConnectionCommand, yi as AddNodeCommand, z as removePreviewToken } from "./users.store-YX3xGXV-.js";
import { Gc as CANVAS_ZOOMED_VIEW_EXPERIMENT, Ia as resolveNodeWebhookId, Oa as isHitlToolType, Pn as MCP_TRIGGER_NODE_TYPE, Sr as UPDATE_WEBHOOK_ID_NODE_TYPES, Uo as v4_default, Vi as require_cloneDeep, Xa as deepCopy, Yi as require__baseUniq, Za as isCommunityPackageName, _a as getNodeInputs, _c as LOCAL_STORAGE_LOGS_SYNC_SELECTION, ga as getConnectionTypes, gc as LOCAL_STORAGE_LOGS_PANEL_OPEN, gn as FORM_TRIGGER_NODE_TYPE, gr as STICKY_NODE_TYPE, hc as LOCAL_STORAGE_LOGS_PANEL_DETAILS_PANEL_SUB_NODE, ho as NodeConnectionTypes, mc as LOCAL_STORAGE_LOGS_PANEL_DETAILS_PANEL, ml as EnterpriseEditionFeature, ms as VIEWS, pa as generateNodesGraph, rl as NDV_IN_FOCUS_PANEL_EXPERIMENT, va as getNodeOutputs, wa as getSubworkflowId, wr as WEBHOOK_NODE_TYPE, ya as getNodeParameters } from "./constants-sixw52iU.js";
import { j as defineStore, r as useRootStore } from "./_baseOrderBy-BrADeb8X.js";
import { t as useExecutionsStore } from "./executions.store-BOXiu41e.js";
import { t as usePinnedData } from "./usePinnedData-Bs9REe2E.js";
import { a as mapLegacyConnectionToCanvasConnection, c as parseCanvasConnectionHandleString, i as mapCanvasConnectionToLegacyConnection, n as createCanvasConnectionHandleString, o as mapLegacyConnectionsToCanvasConnections } from "./canvas.utils-NT0VdpQR.js";
import { n as useNodeCreatorStore, t as canvasEventBus } from "./canvas.eventBus-BzEuwcnY.js";
import { t as core_default } from "./core-Cm22Y8pb.js";
import { a as bash, i as javascript, n as typescript, r as python, t as xml } from "./xml-D7HpP_o1.js";
...
import { t as clearAllNodeResourceLocatorValues } from "./templateTransforms-DAUIusCv.js";
...
const localStorageSessionIdKey = `n8n-chat/sessionId`;
...
function useCanvasOperations() {
	...
	function toggleNodesPinned(ids, source, { trackHistory = true, trackBulk = true } = {}) {
		if (trackHistory && trackBulk) historyStore.startRecordingUndo();
		const pinnableNodesWithPinnedData = workflowsStore.getNodesByIds(ids).map((node) => ({
			node,
	...
	function getNodesToSave(nodes) {
		...
		for (const node of nodes) {
			...
			const workflowDocumentStore = workflowsStore.workflowId ? useWorkflowDocumentStore(createWorkflowDocumentId(workflowsStore.workflowId)) : null;
			const pinDataForNode = workflowDocumentStore ? pinDataToExecutionData(workflowDocumentStore.pinData)[node.name] : void 0;
			if (pinDataForNode) data.pinData[node.name] = pinDataForNode;
			if (nodeSaveData.credentials && settingsStore.isEnterpriseFeatureEnabled[EnterpriseEditionFeature.Sharing]) nodeSaveData.credentials = filterAllowedCredentials(nodeSaveData.credentials, workflowDocumentStore?.usedCredentials ?? {});
	...
	async function openExecution(executionId, nodeId) {
		...
		if (!["manual", "evaluation"].includes(data.mode)) {
			if (workflowsStore.workflowId) useWorkflowDocumentStore(createWorkflowDocumentId(workflowsStore.workflowId)).setPinData({});
		}
		if (nodeId) {
			const node = workflowsStore.getNodeById(nodeId);
	...
	async function openWorkflowTemplate(templateId) {
		...
		useWorkflowDocumentStore(createWorkflowDocumentId(workflowsStore.workflowId)).addToMeta({ templateId: `${templateId}` });
...
export { useChat as S, MessageWithButtons_default as _, LOCAL_STORAGE_PANEL_HEIGHT as a, useI18n$1 as b, LOG_DETAILS_PANEL_STATE as c, parseBotChatMessageContent as d, shouldBlockUserInput as f, MessageComponentKey as g, localStorageSessionIdKey as h, LOCAL_STORAGE_OVERVIEW_PANEL_WIDTH as i, chatEventBus as l, ChatSymbol as m, useExperimentalNdvStore as n, LOCAL_STORAGE_PANEL_WIDTH as o, ChatOptionsSymbol as p, useLogsStore as r, LOGS_PANEL_STATE as s, useCanvasOperations as t, constructChatWebsocketUrl as u, MarkdownRenderer_default as v, useOptions as x, Button_default as y };
