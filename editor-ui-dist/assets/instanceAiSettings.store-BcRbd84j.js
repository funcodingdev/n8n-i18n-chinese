import { Bt as toRaw, It as ref, Pt as reactive, S as computed, gt as watch } from "./vue.runtime.esm-bundler-DYHsQBZB.js";
import { i as i18n } from "./src-C3h3FWXj.js";
import { on as deriveInstanceAiSetupState } from "./src-BrbN-vPn.js";
...
async function fetchModelCatalog(context) {
	return await makeRestApiRequest(context, "GET", "/instance-ai/settings/models");
}
async function verifyModel(context, body) {
	return await makeRestApiRequest(context, "POST", "/instance-ai/settings/verify/model", body);
}
async function verifySandbox(context, body) {
	return await makeRestApiRequest(context, "POST", "/instance-ai/settings/verify/sandbox", body);
}
async function verifySearch(context, body) {
	return await makeRestApiRequest(context, "POST", "/instance-ai/settings/verify/search", body);
}
//#endregion
//#region src/features/ai/instanceAi/instanceAiConfiguration.ts
var SEARCH_STATE_BY_SOURCE = {
	ui: "set",
	env: "env",
	disabled: "disabled",
	none: "notset"
};
function deriveInstanceAiConfiguration(settings, modelCredentials, serviceCredentials) {
	const state = settings ? deriveInstanceAiSetupState(settings) : null;
	const modelCredential = modelCredentials.find((credential) => credential.id === settings?.modelCredentialId);
	const searchCredential = serviceCredentials.find((credential) => credential.id === settings?.searchCredentialId);
	const modelConfigured = state !== null && state.modelSource !== "none";
	const sandboxConfigured = state !== null && state.sandboxSource !== "none";
	const searchState = state ? SEARCH_STATE_BY_SOURCE[state.webSearchSource] : "notset";
	return {
		modelCredential,
		modelConfigured,
...
var useInstanceAiSettingsStore = defineStore("instanceAiSettings", () => {
	...
	*/
	const isInstanceAiSetupPanelEnabled = computed(() => settingsStore.moduleSettings?.["instance-ai"]?.instanceAiSetupPanelEnabled === true);
	function syncInstanceAiFlagIntoGlobalModuleSettings(adminRes) {
		...
		const configuration = deriveInstanceAiConfiguration(adminRes, instanceModelCredentials.value, serviceCredentials.value);
		const merged = {
	...
	*/
	async function save(showToast = true) {
		...
		try {
			...
			clearDraft();
			if (showToast) toast.showMessage({
	...
	async function persistEnabled(value, showToast = true) {
		isSaving.value = true;
		try {
	...
	const computerUseConnectionStatus = computed(() => {
	...
	const browserUseConnectionStatus = computed(() => {
	...
	*/
	...
	async function loadModelCatalog() {
		...
		const request = fetchModelCatalog(rootStore.restApiContext).then((response) => {
			if (Object.values(response.models).some((models) => models.length > 0)) modelCatalog.value = response.models;
		}).catch(() => {}).finally(() => {
	...
	async function verifyModel$1(payload) {
		return await verifyModel(rootStore.restApiContext, payload);
	}
	async function verifySandbox$1(payload) {
		return await verifySandbox(rootStore.restApiContext, payload);
	}
	async function verifySearch$1(payload) {
		return await verifySearch(rootStore.restApiContext, payload);
	}
	return {
		canManage,
		canManageAiUsage,
		...
		verifySandbox: verifySandbox$1,
		verifySearch: verifySearch$1,
		browserConnected,
...
//#endregion
export { postCancel as a, postFeedback as c, getInstanceAiCredits as i, postMessage as l, deriveInstanceAiConfiguration as n, postCancelTask as o, ensureThread as r, postConfirmation as s, useInstanceAiSettingsStore as t };
