...
import { $ as openBlock, A as createTextVNode, C as createBaseVNode, E as createElementBlock, Ft as ref, N as defineComponent, O as createSlots, S as computed, Sn as toDisplayString, T as createCommentVNode, W as nextTick, Wt as unref, X as onMounted, _ as Fragment, _n as normalizeClass, bt as withDirectives, f as vModelText, gt as watch, h as withModifiers, it as renderSlot, j as createVNode, m as withKeys, rt as renderList, st as resolveDynamicComponent, w as createBlock, yt as withCtx } from "./vue.runtime.esm-bundler-Bw8GKr4Y.js";
import { s as useI18n } from "./src-M_cPVtop.js";
import { Ca as N8nHeading_default, Da as N8nIcon_default, E as Switch_default, Ea as N8nButton_default, Ii as Input_default, N as N8nRadioButtons_default, St as N8nCollapsiblePanel_default, Ta as N8nText_default, _t as N8nSelect_default, vt as N8nOption_default, wa as N8nCallout_default, wt as N8nCard_default } from "./src-D7XhL4SE.js";
import { t as _plugin_vue_export_helper_default } from "./_plugin-vue_export-helper-Da88TEg1.js";
import { O as require___vite_browser_external } from "./empty-DIo9Sq4M.js";
import { Gr as useProjectsStore, nt as useCredentialsStore } from "./workflows.store-DQQfIwKi.js";
import { ao as AGENT_SCHEDULE_TRIGGER_TYPE, el as getResourcePermissions } from "./constants-Brs44Mbc.js";
...
import { t as CredentialsDropdown_default } from "./CredentialsDropdown-BczSki51.js";
import { A as updateScheduleIntegration, a as activateScheduleIntegration, b as getScheduleIntegration, d as createSlackAgentApp, f as deactivateScheduleIntegration, x as getSlackAgentAppManifest } from "./agentTelemetry.utils-JiK0HuJn.js";
import { n as useAgentIntegrationStatus, r as useAgentIntegrationsCatalog } from "./useAgentIntegrationStatus-HnCb6P0Z.js";
import { n as useAgentConfirmationModal, t as useAgentPublish } from "./useAgentPublish-DpfDUSFt.js";
...
//#endregion
//#region ../../../node_modules/.pnpm/cron@4.4.0/node_modules/cron/dist/constants.js
...
//#endregion
//#region ../../../node_modules/.pnpm/cron@4.4.0/node_modules/cron/dist/errors.js
var require_errors = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var ExclusiveParametersError = class extends CronError {
...
//#endregion
//#region ../../../node_modules/.pnpm/cron@4.4.0/node_modules/cron/dist/time.js
var require_time = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	exports.CronTime = class CronTime {
		constructor(source, timeZone, utcOffset) {
			...
			this.month = {};
			this.dayOfWeek = {};
			if (timeZone != null && utcOffset != null) throw new errors_1.ExclusiveParametersError("timeZone", "utcOffset");
			if (timeZone) {
				if (!luxon_1.DateTime.fromObject({}, { zone: timeZone }).isValid) throw new errors_1.CronError("Invalid timezone.");
				this.timeZone = timeZone;
			}
			if (utcOffset != null) this.utcOffset = utcOffset;
			if (timeZone == null && utcOffset == null) this.timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
		...
		sendAt(i) {
			let date = this.realDate && this.source instanceof luxon_1.DateTime ? this.source : luxon_1.DateTime.utc();
			...
			if (this.utcOffset !== void 0) {
				...
				const utcZone = `UTC${sign}${offsetHoursStr}:${String(offsetMins).padStart(2, "0")}`;
				date = date.setZone(utcZone);
				if (!date.isValid) throw new errors_1.CronError("ERROR: You specified an invalid UTC offset.");
			}
			if (this.realDate) {
				if (luxon_1.DateTime.local() > date) throw new errors_1.CronError("WARNING: Date in past. Will never be fired.");
				return date;
			}
			if (i === void 0 || isNaN(i) || i < 0) return this.getNextDateFrom(date);
			else {
				const dates = [];
				for (; i > 0; i--) {
		...
		getNextDateFrom(start, timeZone) {
			var _a, _b;
			if (start instanceof Date) start = luxon_1.DateTime.fromJSDate(start);
			if (timeZone) start = start.setZone(timeZone);
			else timeZone = (_a = start.zone.zoneName) !== null && _a !== void 0 ? _a : start.zone.fixed;
			let date = luxon_1.DateTime.fromFormat(`${start.year}-${start.month}-${start.day} ${start.hour}:${start.minute}:${start.second}`, "yyyy-M-d H:m:s", { zone: "UTC" });
			const firstDate = date.toMillis();
			if (!this.realDate) {
				if (date.millisecond > 0) date = date.set({
					millisecond: 0,
					second: date.second + 1
				});
			}
			if (!date.isValid) throw new errors_1.CronError("ERROR: You specified an invalid date.");
			const maxMatch = luxon_1.DateTime.now().plus({ years: 8 });
			while (true) {
				if (date > maxMatch) throw new errors_1.CronError(`Something went wrong. No execution date was found in the next 8 years.
				...
				if (!(date.day in this.dayOfMonth) && Object.keys(this.dayOfMonth).length !== 31 && !(this._getWeekDay(date) in this.dayOfWeek && Object.keys(this.dayOfWeek).length !== 7) || !(this._getWeekDay(date) in this.dayOfWeek) && Object.keys(this.dayOfWeek).length !== 7 && !(date.day in this.dayOfMonth && Object.keys(this.dayOfMonth).length !== 31)) {
				...
				if (date.toMillis() === firstDate || !(date.second in this.second) && Object.keys(this.second).length !== 60) {
			...
			date = luxon_1.DateTime.fromFormat(`${date.year}-${date.month}-${date.day} ${date.hour}:${date.minute}:${date.second}`, "yyyy-M-d H:m:s", { zone: timeZone });
			const nonDSTReferenceDate = luxon_1.DateTime.fromFormat(`${date.year}-1-1 0:0:0`, "yyyy-M-d H:m:s", { zone: timeZone });
			if ((expectedHour !== date.hour || expectedMinute !== date.minute) && nonDSTReferenceDate.offset !== date.offset) {
				while (date.minus({ minute: 1 }).offset !== nonDSTReferenceDate.offset) date = date.minus({ minute: 1 });
				return date;
			}
			const hourTestDate = date.minus({ hour: 1 });
			const twoHourTestDate = date.minus({ hour: 2 });
			if ((hourTestDate.hour === date.hour || twoHourTestDate.hour === hourTestDate.hour) && hourTestDate > start) date = hourTestDate;
			const halfHourTestDate = date.minus({ minute: 30 });
			if ((halfHourTestDate.minute === date.minute || hourTestDate.minute === halfHourTestDate.minute) && halfHourTestDate > start) date = halfHourTestDate;
			return date;
		}
		_wcOrAll(unit) {
		...
		_hasAll(unit) {
			...
			for (let i = low, n = high; i < n; i++) if (!(i in this[unit])) return false;
			return true;
		}
		_parse(source) {
			var _a;
			source = source.toLowerCase();
			if (Object.keys(constants_1.PRESETS).includes(source)) source = constants_1.PRESETS[source];
			source = source.replace(/[a-z]{1,3}/gi, (alias) => {
				if (Object.keys(constants_1.ALIASES).includes(alias)) return constants_1.ALIASES[alias].toString();
				throw new errors_1.CronError(`Unknown alias: ${alias}`);
			});
			const units = source.trim().split(/\s+/);
			if (units.length < constants_1.TIME_UNITS_LEN - 1) throw new errors_1.CronError("Too few fields");
			if (units.length > constants_1.TIME_UNITS_LEN) throw new errors_1.CronError("Too many fields");
			const unitsLen = units.length;
			for (const unit of constants_1.TIME_UNITS) {
				const cur = (_a = units[constants_1.TIME_UNITS.indexOf(unit) - (constants_1.TIME_UNITS_LEN - unitsLen)]) !== null && _a !== void 0 ? _a : constants_1.PARSE_DEFAULTS[unit];
		...
		_parseField(value, unit) {
			...
			value.split(",").forEach((field) => {
				...
				if (wildcardIndex !== -1 && wildcardIndex !== 0) throw new errors_1.CronError(`Field (${field}) has an invalid wildcard expression`);
			});
			...
			for (const range of allRanges) {
				...
				if ((match === null || match === void 0 ? void 0 : match[1]) !== void 0) {
					...
					if (step === 0) throw new errors_1.CronError(`Field (${unit}) has a step of zero`);
					if (upper !== void 0 && lower > upper) throw new errors_1.CronError(`Field (${unit}) has an invalid range`);
					if (lower < low || upper !== void 0 && upper > high || upper === void 0 && lower > high) throw new errors_1.CronError(`Field value (${value}) is out of range`);
					lower = Math.min(Math.max(low, ~~Math.abs(lower)), high);
					if (upper !== void 0) upper = Math.min(high, ~~Math.abs(upper));
					else upper = wasStepDefined ? high : lower;
					pointer = lower;
					do {
				...
				} else throw new errors_1.CronError(`Field (${unit}) cannot be parsed`);
			}
		}
	};
}));
//#endregion
//#region ../../../node_modules/.pnpm/cron@4.4.0/node_modules/cron/dist/job.js
var require_job = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var child_process_1 = require___vite_browser_external();
	var errors_1 = require_errors();
	var time_1 = require_time();
	exports.CronJob = class CronJob {
		get isActive() {
		...
		constructor(cronTime, onTick, onComplete, start, timeZone, context, runOnInit, utcOffset, unrefTimeout, waitForCompletion, errorHandler, name, threshold) {
			...
			if (timeZone != null && utcOffset != null) throw new errors_1.ExclusiveParametersError("timeZone", "utcOffset");
			if (timeZone != null) this.cronTime = new time_1.CronTime(cronTime, timeZone, null);
			else if (utcOffset != null) this.cronTime = new time_1.CronTime(cronTime, null, utcOffset);
			else this.cronTime = new time_1.CronTime(cronTime, timeZone, utcOffset);
			if (unrefTimeout != null) this.unrefTimeout = unrefTimeout;
			if (onComplete != null) this.onComplete = this._fnWrap(onComplete);
			if (threshold != null) this.threshold = Math.abs(threshold);
		...
		static from(params) {
			if (params.timeZone != null && params.utcOffset != null) throw new errors_1.ExclusiveParametersError("timeZone", "utcOffset");
			if (params.timeZone != null) return new CronJob(params.cronTime, params.onTick, params.onComplete, params.start, params.timeZone, params.context, params.runOnInit, params.utcOffset, params.unrefTimeout, params.waitForCompletion, params.errorHandler, params.name, params.threshold);
			else if (params.utcOffset != null) return new CronJob(params.cronTime, params.onTick, params.onComplete, params.start, null, params.context, params.runOnInit, params.utcOffset, params.unrefTimeout, params.waitForCompletion, params.errorHandler, params.name, params.threshold);
			else return new CronJob(params.cronTime, params.onTick, params.onComplete, params.start, params.timeZone, params.context, params.runOnInit, params.utcOffset, params.unrefTimeout, params.waitForCompletion, params.errorHandler, params.name, params.threshold);
		}
		_fnWrap(cmd) {
			var _a, _b;
			switch (typeof cmd) {
				case "function": return cmd;
				case "string": {
					const [command, ...args] = cmd.split(" ");
					return child_process_1.spawn.bind(void 0, command !== null && command !== void 0 ? command : cmd, args, {});
				}
				case "object": return child_process_1.spawn.bind(void 0, cmd.command, (_a = cmd.args) !== null && _a !== void 0 ? _a : [], (_b = cmd.options) !== null && _b !== void 0 ? _b : {});
			}
		}
		...
		setTime(time) {
			if (!(time instanceof time_1.CronTime)) throw new errors_1.CronError("time must be an instance of CronTime.");
			...
			return __awaiter(this, void 0, void 0, function* () {
				...
				try {
					for (const callback of this._callbacks) {
						...
						if (result && typeof result === "object" && typeof result.then === "function") if (this.waitForCompletion) yield result;
						else result.catch((error) => {
		...
		start() {
			...
			if (timeout >= 0) {
				...
				if (absoluteTimeout <= this.threshold) {
					...
					this.lastExecution = /* @__PURE__ */ new Date();
					this.fireOnTick();
				} else console.warn(`${message}. Skipping execution as it exceeds threshold (${this.threshold}ms).`);
		...
		_waitForJobCompletion() {
			return __awaiter(this, void 0, void 0, function* () {
				while (this._isCallbackRunning) yield new Promise((resolve) => setTimeout(resolve, 100));
			});
		}
		stop() {
...
//#endregion
//#region src/features/agents/utils/schedulePresets.ts
...
function getSchedulePresetByCronExpression(cronExpression) {
	return schedulePresets.find((preset) => preset.cronExpression === cronExpression.trim());
}
function getScheduleInputMode(cronExpression) {
	return getSchedulePresetByCronExpression(cronExpression) ? "preset" : "custom";
}
function getNextScheduleOccurrence(cronExpression, timezone) {
	if (!cronExpression.trim()) return null;
	try {
		return new import_dist.CronTime(cronExpression, timezone).sendAt().toJSDate();
	} catch {
		return null;
	}
}
...
var AgentScheduleTriggerCard_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		const scheduleInputMode = ref("preset");
		...
		const scheduleInputModeOptions = computed(() => [{
			label: locale.baseText("agents.schedule.cron.mode.preset"),
			value: "preset"
		}, {
			label: locale.baseText("agents.schedule.cron.mode.custom"),
			value: "custom"
		}]);
		const schedulePresetOptions = computed(() => schedulePresets.map((preset) => ({
			label: locale.baseText(preset.labelKey),
			value: preset.cronExpression
		})));
		const selectedPresetCronExpression = computed(() => getSchedulePresetByCronExpression(cronExpression.value)?.cronExpression ?? "");
		const nextScheduleOccurrence = computed(() => getNextScheduleOccurrence(nextOccurrenceCronExpression.value, rootStore.timezone));
		...
		function applyConfig(config) {
			...
			if (scheduleInputMode.value === "custom") lastCustomCronExpression.value = config.cronExpression;
		...
		function onCancel() {
			...
			if (scheduleInputMode.value === "custom") lastCustomCronExpression.value = lastSavedCronExpression.value;
		...
		function onScheduleInputModeInput(value) {
			if (value === scheduleInputMode.value) return;
			if (scheduleInputMode.value === "custom") lastCustomCronExpression.value = cronExpression.value;
			scheduleInputMode.value = value;
			if (value === "preset") {
				cronExpression.value = getSchedulePresetByCronExpression(cronExpression.value)?.cronExpression ?? schedulePresets[0]?.cronExpression ?? "";
				nextOccurrenceCronExpression.value = cronExpression.value;
		...
		function onSchedulePresetInput(value) {
			cronExpression.value = value;
			...
			return openBlock(), createBlock(resolveDynamicComponent(__props.flat ? "div" : unref(N8nCard_default)), { class: normalizeClass(__props.flat ? _ctx.$style.flatBody : _ctx.$style.card) }, createSlots({
				default: withCtx(() => [createBaseVNode("div", { class: normalizeClass(_ctx.$style.cardBody) }, [
					...
					createBaseVNode("div", { class: normalizeClass(_ctx.$style.field) }, [
						createBaseVNode("div", { class: normalizeClass(_ctx.$style.fieldHeader) }, [createVNode(unref(N8nText_default), {
							size: "small",
							bold: ""
						}, {
							...
							"model-value": scheduleInputMode.value,
							options: scheduleInputModeOptions.value,
							size: "small",
							disabled: loading.value || saving.value,
							"data-testid": "schedule-cron-mode-toggle",
							"onUpdate:modelValue": onScheduleInputModeInput
						}, null, 8, [
							"model-value",
							"options",
							"disabled"
						])], 2),
						scheduleInputMode.value === "preset" ? (openBlock(), createBlock(unref(N8nSelect_default), {
							key: 0,
							"model-value": selectedPresetCronExpression.value,
							disabled: loading.value || saving.value,
							class: normalizeClass(_ctx.$style.cronSelect),
							size: "small",
							"data-testid": "schedule-preset-select",
							"onUpdate:modelValue": onSchedulePresetInput
						}, {
							default: withCtx(() => [(openBlock(true), createElementBlock(Fragment, null, renderList(schedulePresetOptions.value, (preset) => {
						...
						nextScheduleOccurrenceText.value ? (openBlock(), createBlock(unref(N8nText_default), {
							key: 2,
							class: normalizeClass(_ctx.$style.helpText),
							size: "small",
							"data-testid": "schedule-next-occurrence"
						}, {
							default: withCtx(() => [createTextVNode(toDisplayString(unref(locale).baseText("agents.schedule.nextOccurrence", { interpolate: { occurrence: nextScheduleOccurrenceText.value } })), 1)]),
							_: 1
						}, 8, ["class"])) : (openBlock(), createElementBlock("div", {
							key: 3,
							class: normalizeClass(_ctx.$style.fieldDescriptionSpacer)
						}, null, 2)),
...
var AgentScheduleTriggerCard_vue_vue_type_style_index_0_lang_module_default = {
	card: "_card_w7ek8_2",
	flatBody: "_flatBody_w7ek8_7",
	cardHeader: "_cardHeader_w7ek8_11",
	statusRow: "_statusRow_w7ek8_17",
	statusDot: "_statusDot_w7ek8_23",
	statusConnected: "_statusConnected_w7ek8_30",
	statusDisconnected: "_statusDisconnected_w7ek8_34",
	statusLabel: "_statusLabel_w7ek8_38",
	cardBody: "_cardBody_w7ek8_42",
	description: "_description_w7ek8_48",
	helpText: "_helpText_w7ek8_49",
	field: "_field_w7ek8_53",
	fieldHeader: "_fieldHeader_w7ek8_59",
	cronSelect: "_cronSelect_w7ek8_67",
	toggleRow: "_toggleRow_w7ek8_75",
	actions: "_actions_w7ek8_82",
	saveActions: "_saveActions_w7ek8_89",
	errorText: "_errorText_w7ek8_96",
	fieldDescriptionSpacer: "_fieldDescriptionSpacer_w7ek8_99"
};
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(Modal_default, {
				...
				content: withCtx(() => [createBaseVNode("div", { class: normalizeClass(_ctx.$style.body) }, [
					...
					!selectedTriggerType.value ? (openBlock(), createElementBlock("div", {
						...
						!unref(isConnected)(currentIntegration.value.type) && currentIntegration.value.type !== "slack" ? (openBlock(), createBlock(AgentIntegrationCredentialConnection_default, {
							...
							onEdit: _cache[4] || (_cache[4] = ($event) => onEditCredential(currentIntegration.value.type))
						}, null, 8, [
							"modelValue",
							"integration-type",
							"integration-label",
							...
							onCreate: _cache[5] || (_cache[5] = ($event) => onCreateCredential(currentIntegration.value)),
							onDisconnect: _cache[6] || (_cache[6] = ($event) => onDisconnect(currentIntegration.value.type))
						}, null, 8, [
							"model-value",
							"integration-type",
							"integration-label",
						...
						currentIntegration.value.type !== "slack" || !unref(isConnected)(currentIntegration.value.type) ? (openBlock(), createBlock(AgentIntegrationSettingsForm_default, {
							...
							fn: withCtx(() => [createVNode(AgentIntegrationCredentialConnection_default, {
								...
								"onUpdate:modelValue": _cache[7] || (_cache[7] = ($event) => selectedCredentials.value[currentIntegration.value.type] = $event),
								onCreate: _cache[8] || (_cache[8] = ($event) => onCreateCredential(currentIntegration.value)),
								onEdit: _cache[9] || (_cache[9] = ($event) => onEditCredential(currentIntegration.value.type)),
								onConnect: _cache[10] || (_cache[10] = ($event) => onConnect(currentIntegration.value.type)),
								onDisconnect: _cache[11] || (_cache[11] = ($event) => onDisconnect(currentIntegration.value.type))
							}, null, 8, [
								"model-value",
								"integration-type",
								"integration-label",
						...
						currentIntegration.value.type !== "slack" && !unref(isConnected)(currentIntegration.value.type) && hasError(currentIntegration.value.type) ? (openBlock(), createBlock(unref(N8nText_default), {
							...
							default: withCtx(() => [createTextVNode(toDisplayString(unref(errorMessages)[currentIntegration.value.type]) + " ", 1), selectedCredentials.value[currentIntegration.value.type] && !unref(errorIsConflict)[currentIntegration.value.type] ? (openBlock(), createElementBlock("a", {
								...
								class: normalizeClass(_ctx.$style.link),
								href: "#",
								onClick: _cache[12] || (_cache[12] = withModifiers(($event) => onEditCredential(currentIntegration.value.type), ["prevent"]))
							}, toDisplayString(unref(i18n).baseText("agents.builder.addTrigger.editCredential")), 3)) : createCommentVNode("", true)]),
							_: 1
						}, 8, ["class"])) : createCommentVNode("", true)
					], 2)) : createCommentVNode("", true)
				...
				fn: withCtx(() => [createBaseVNode("div", { class: normalizeClass(_ctx.$style.footer) }, [createBaseVNode("div", { class: normalizeClass(_ctx.$style.footerActions) }, [!unref(isConnected)(currentIntegration.value.type) ? (openBlock(), createBlock(unref(N8nButton_default), {
					key: 0,
					variant: "solid",
					disabled: !selectedCredentials.value[currentIntegration.value.type] || isLoading(currentIntegration.value.type) || unref(publishing) || !!settingsFormRef.value?.validationError,
					loading: isLoading(currentIntegration.value.type) || unref(publishing),
					size: "small",
					"data-testid": `${currentIntegration.value.type}-connect-button`,
					onClick: _cache[13] || (_cache[13] = ($event) => onConnect(currentIntegration.value.type))
				}, {
					prefix: withCtx(() => [createVNode(unref(N8nIcon_default), {
						icon: "plug",
						size: "xsmall"
					...
					onClick: _cache[14] || (_cache[14] = ($event) => onDisconnect(currentIntegration.value.type))
				}, {
					prefix: withCtx(() => [createVNode(unref(N8nIcon_default), {
						icon: "unlink",
						size: "xsmall"
...
var AgentAddTriggerModal_vue_vue_type_style_index_0_lang_module_default = {
	body: "_body_1gg49_125",
	pickerRow: "_pickerRow_1gg49_132",
	triggerPicker: "_triggerPicker_1gg49_138",
	triggerPickerWithIcon: "_triggerPickerWithIcon_1gg49_144",
	optionRow: "_optionRow_1gg49_148",
	optionIcon: "_optionIcon_1gg49_156",
	optionLabel: "_optionLabel_1gg49_160",
	placeholderState: "_placeholderState_1gg49_167",
	integrationConfig: "_integrationConfig_1gg49_177",
	modalDescription: "_modalDescription_1gg49_183",
	footer: "_footer_1gg49_187",
	footerActions: "_footerActions_1gg49_193",
	connectedSection: "_connectedSection_1gg49_199",
	errorText: "_errorText_1gg49_205",
	link: "_link_1gg49_209",
	webhookRow: "_webhookRow_1gg49_216",
	webhookInput: "_webhookInput_1gg49_222"
};
