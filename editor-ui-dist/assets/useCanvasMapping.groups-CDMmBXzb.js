import { Bi as CANVAS_NODE_GROUP_HANDLE_LEFT, Hi as CANVAS_NODE_GROUP_TYPE, Ki as createCanvasGroupNodeId, Vi as CANVAS_NODE_GROUP_HANDLE_RIGHT, kr as DEFAULT_NODE_SIZE, mr as createCanvasConnectionId, ur as applyOffset } from "./workflows.store-CEslMXyQ.js";
import "./src-D3mf01t5.js";
import { Hn as STICKY_NODE_TYPE } from "./constants-DCcvObnS.js";
...
var GROUP_DESCRIPTION_MIN_ZOOM = .66;
/**
...
*/
var GROUP_NODE_Z_INDEX_EXPANDED = -1e4;
//#endregion
...
function mapGroupsToVueFlowNodes({ allGroups, getNodeById, getNodeDisplaySize, getGroupVisualOffset, isGroupCollapsed, readOnly, getNodeExecutionSnapshot }) {
	...
	for (const group of allGroups) {
		...
		const connectableMembers = group.nodeIds.map(getNodeById).filter((node) => node !== void 0).filter((node) => node.type !== STICKY_NODE_TYPE);
		const data = {
			group,
			nodesRect,
			isCollapsed: collapsed,
			executionStatus: aggregateGroupExecution(group.nodeIds, getNodeExecutionSnapshot),
			allNodesDisabled: connectableMembers.length > 0 && connectableMembers.every((node) => node.disabled === true)
		};
		...
		out.push({
			...
			selectable: true,
			connectable: false,
			zIndex: collapsed ? -1 : GROUP_NODE_Z_INDEX_EXPANDED,
			data
		});
	}
	return out;
...
//#endregion
export { mapGroupsToVueFlowNodes as a, GROUP_DESCRIPTION_MIN_ZOOM as c, computeNodesRectFromStore as i, GROUP_HEADER_HEIGHT as l, buildCollapsedGroupByNodeId as n, remapCollapsedGroupConnections as o, computeGroupFrameRects as r, titleBarFromNodesRect as s, aggregateGroupExecution as t };
