...
import { Gt as N8nCallout_default, It as N8nOption_default, Ma as FocusScope_default, Pt as N8nSelect_default, S as N8nInlineTextEdit_default, V as Switch_default, _ as DialogHeader_default, co as N8nIconButton_default, h as Dialog_default, lo as N8nButton_default, m as DialogFooter_default, ra as N8nTooltip_default, ro as N8nText_default, so as Input_default, uo as N8nIcon_default } from "./src-DtxHbF5I.js";
...
import { tt as UNSUPPORTED_AGENT_NODE_TOOL_OPERATIONS } from "./src-7oOw7OxE.js";
...
import { an as EditorView, kn as EditorState, r as javascript, vn as lineNumbers } from "./expressions-DxERK7e0.js";
import { l as codeEditorTheme } from "./RunData-Dr61zBOd.js";
import { t as NodeIcon_default } from "./NodeIcon-jIR1F-Qq.js";
import { c as updateToolRefFromNode, l as updateWorkflowToolRef, r as nodeToMcpServer, s as toolRefToNode } from "./useMcpServerAdapter-BXUGH-sM.js";
import { t as NodeToolSettingsContent_default } from "./NodeToolSettingsContent-C3bNbVqt.js";
import { r as useAgentToolCatalog } from "./useAgentToolCatalog-_UAAbpVY.js";
...
var WorkflowToolConfigContent_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*  lost its supported trigger (the fetch filters on trigger type). */
		const isMissing = computed(() => !isLoadingWorkflows.value && workflow.value.length > 0 && !projectWorkflows.value.some((candidate) => candidate.name === workflow.value));
		/** Target still exists but is archived or holds a node that can't run as a tool. */
		const isUnusable = computed(() => !isLoadingWorkflows.value && !isMissing.value && workflow.value.length > 0 && !availableWorkflows.value.some((candidate) => candidate.name === workflow.value));
		/** Workflow names aren't unique, and the backend resolves the ref by name. */
		const isAmbiguous = computed(() => projectWorkflows.value.filter((candidate) => candidate.name === workflow.value).length > 1);
		/**
		...
		*/
		...
		const targetWorkflowId = computed(() => workflowOptions.value.find((option) => option.name === workflow.value)?.id);
				...
				createBaseVNode("div", { class: normalizeClass(_ctx.$style.field) }, [
					...
					createBaseVNode("div", { class: normalizeClass(_ctx.$style.targetRow) }, [
						...
						mode.value === "list" ? (openBlock(), createBlock(unref(N8nSelect_default), {
							...
							default: withCtx(() => [isMissing.value || isUnusable.value ? (openBlock(), createBlock(unref(N8nOption_default), {
								key: workflow.value,
