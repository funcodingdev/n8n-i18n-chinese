...
import { Dr as isValidNodeConnectionType, Ei as getTriggerNodeServiceName, G as isTutorialTemplateId, Lt as useWorkflowsListStore, Qt as useNodeHelpers, Ri as parse, Ut as injectWorkflowDocumentStore, V as SampleTemplates, Vt as createWorkflowDocumentId, Wt as useWorkflowDocumentStore, _ as openFormPopupWindow, a as createExecutionDataId, b as useNodeTypesStore, c as clearPopupWindowState, h as getExecutionErrorToastConfiguration, i as useWorkflowExecutionStateStore, kt as useWorkflowHelpers, m as getExecutionErrorMessage, n as createWorkflowExecutionStateId, nt as useCredentialsStore, o as useExecutionDataStore, t as useWorkflowsStore } from "./workflows.store-DQQfIwKi.js";
import { t as useToast } from "./useToast-DMjrW7lG.js";
import { Ks as isCommunityPackageName, Pc as TRIMMED_TASK_DATA_CONNECTIONS_KEY, Qi as WORKFLOW_SETTINGS_MODAL_KEY, Tc as EVALUATION_TRIGGER_NODE_TYPE, cs as generateNodesGraph, rs as createRunExecutionData } from "./constants-Brs44Mbc.js";
import { T as defineStore, t as useRootStore } from "./useRootStore-NTSmdx9S.js";
import { at as sendGetWorkerStatus, t as useSettingsStore } from "./settings.store-CrL-o4BC.js";
...
import { r as useWorkflowSaving, t as useBuilderStore } from "./builder.store-DNbDO--p.js";
import { t as useDocumentTitle } from "./useDocumentTitle-BoZ4pEPK.js";
import { i as globalLinkActionsEventBus, o as codeNodeEditorEventBus } from "./event-bus-C9EFPlnM.js";
...
import { t as useActivationError } from "./useActivationError-Bu7Jyyuv.js";
import { t as useReadyToRunStore } from "./readyToRun.store-Drlkg-kJ.js";
...
function setRunExecutionData(execution, runExecutionData, workflowState) {
	const stateStore = useWorkflowExecutionStateStore(createWorkflowExecutionStateId(useWorkflowsStore().workflowId));
...
async function executionRecovered({ data }, options) {
	const stateStore = useWorkflowExecutionStateStore(createWorkflowExecutionStateId(useWorkflowsStore().workflowId));
	const uiStore = useUIStore();
	if (typeof stateStore.activeExecutionId === "undefined") return;
	uiStore.setProcessingExecutionResults(true);
	const execution = await fetchExecutionData(data.executionId);
	if (!execution) {
		uiStore.setProcessingExecutionResults(false);
...
async function executionStarted({ data }) {
	const workflowsStore = useWorkflowsStore();
	const stateStore = useWorkflowExecutionStateStore(createWorkflowExecutionStateId(workflowsStore.workflowId));
	const isIframe = window !== window.parent;
	if (typeof stateStore.activeExecutionId === "undefined" && !isIframe) return;
	const needsInit = stateStore.activeExecutionId === null || typeof stateStore.activeExecutionId === "undefined" || isIframe && stateStore.activeExecutionId !== data.executionId;
	if (needsInit) stateStore.promotePendingExecution(data.executionId);
