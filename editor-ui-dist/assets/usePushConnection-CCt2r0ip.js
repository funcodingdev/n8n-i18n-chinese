...
import { Bt as injectWorkflowState, Er as useToast, Et as useWorkflowHelpers, Ft as useNodeHelpers, G as SampleTemplates, Gr as useWorkflowsListStore, It as useTelemetry, Jr as injectWorkflowDocumentStore, Kr as createWorkflowDocumentId, Or as useExternalHooks, Y as isTutorialTemplateId, Yr as useWorkflowDocumentStore, _n as codeNodeEditorEventBus, at as useCredentialsStore, b as getExecutionErrorToastConfiguration, bn as useWorkflowSaving, da as parse, ea as getTriggerNodeServiceName, hn as globalLinkActionsEventBus, js as useSettingsStore, ln as useBuilderStore, m as clearPopupWindowState, r as useUIStore, s as useWorkflowsStore, sc as sendGetWorkerStatus, vi as isValidNodeConnectionType, w as useNodeTypesStore, y as getExecutionErrorMessage, yn as useDocumentTitle } from "./users.store-DSeNCs16.js";
import { Eo as EVALUATION_TRIGGER_NODE_TYPE, Io as TRIMMED_TASK_DATA_CONNECTIONS_KEY, Ja as isCommunityPackageName, Js as WORKFLOW_SETTINGS_MODAL_KEY, aa as createRunExecutionData, da as generateNodesGraph } from "./constants-DFXtU1Pw.js";
...
import { t as useReadyToRunStore } from "./readyToRun.store-DyR6U5Tv.js";
...
async function workflowAutoDeactivated({ data }) {
	...
	if (workflowsStore.workflowId === data.workflowId) {
		if (!uiStore.stateIsDirty) {
			...
			if (!updatedWorkflow.checksum) throw new Error("Failed to fetch workflow");
			await initializeWorkspace(updatedWorkflow);
		} else documentStore?.value?.setActiveState({
...
async function workflowFailedToActivate({ data }, _options) {
	...
	workflowsStore.setWorkflowInactive(data.workflowId);
	documentStore?.value?.setActiveState({
		activeVersionId: null,
		activeVersion: null
	});
	const toast = useToast();
