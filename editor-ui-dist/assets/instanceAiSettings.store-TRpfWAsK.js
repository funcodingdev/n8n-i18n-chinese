...
var useInstanceAiSettingsStore = defineStore("instanceAiSettings", () => {
	...
	async function fetch() {
		...
		try {
			const [s, p] = await Promise.all([fetchSettings(rootStore.restApiContext), fetchPreferences(rootStore.restApiContext)]);
			settings.value = s;
			preferences.value = p;
	...
	async function save() {
		...
		try {
			...
			const [adminResult, prefsResult] = await Promise.allSettled([hasAdminChanges ? updateSettings(rootStore.restApiContext, draft) : Promise.resolve(settings.value), hasPreferenceChanges ? updatePreferences(rootStore.restApiContext, preferencesDraft) : Promise.resolve(preferences.value)]);
