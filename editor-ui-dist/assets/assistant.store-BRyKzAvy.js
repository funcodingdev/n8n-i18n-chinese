import { It as ref, Nt as onScopeDispose, S as computed, gt as watch } from "./vue.runtime.esm-bundler-Bs4WIMNP.js";
import { x as useI18n } from "./_MapCache-B3VhjNx3.js";
import { g as useRoute } from "./htmlUtils-LDunChtE.js";
import { Dt as useAIAssistantHelpers, E as useWorkflowDocumentStore, S as createWorkflowDocumentId, rn as useNDVStore, t as useWorkflowsStore, vt as useCredentialsStore, y as useWorkflowExecutionStateStore, yt as chatWithAssistant } from "./workflows.store-YK4BoC2f.js";
...
import { Rr as EDITABLE_CANVAS_VIEWS } from "./constants-CQOOvzhs.js";
import { r as useChatPanelStateStore, t as useCodeDiff } from "./useCodeDiff-Bztj_r4N.js";
...
var useAssistantStore = defineStore(STORES.ASSISTANT, () => {
	...
	const clearLastUnreadTimeout = ref(null);
	...
	const hideAssistantFloatingButton = computed(() => EDITABLE_CANVAS_VIEWS.includes(route.name) && !ndvStore.value.activeNode);
	...
	function resetAssistantChat(workflowId) {
		clearMessages();
	...
	function isNodeErrorActive(context) {
		...
		return chatSessionTask.value === "error" && useWorkflowExecutionStateStore(createWorkflowDocumentId(workflowsStore.workflowId)).activeExecutionId === currentSessionActiveExecutionId.value && targetNode === chatSessionError.value?.node.name;
	}
	...
	async function initCredHelp(workflowId, credType) {
		const hasExistingSession = !!currentSessionId.value;
		await initSupportChat(workflowId, locale.baseText("aiAssistant.builder.credentialHelpMessage", { interpolate: { credentialName: credType.displayName } }), credType);
	...
	*/
	async function getVisualContext(workflowId, nodeInfo) {
		...
		const executionResult = useWorkflowExecutionStateStore(createWorkflowDocumentId(workflowsStore.workflowId)).activeExecution?.data?.resultData;
		...
		return {
			...
			currentWorkflow: workflowDataStale.value ? await assistantHelpers.simplifyWorkflowForAssistant(useWorkflowDocumentStore(createWorkflowDocumentId(workflowId)).getSnapshot(), { excludeParameterValues: !allowSendingParameterValues.value }) : void 0,
	...
	async function initSupportChat(workflowId, userMessage, credentialType) {
		resetAssistantChat(workflowId);
		chatSessionTask.value = credentialType ? "credentials" : "support";
		const activeNode = ndvStore.value.activeNode;
		const nodeInfo = assistantHelpers.getNodeInfoForAssistant(workflowId, activeNode, { excludeParameterValues: !allowSendingParameterValues.value });
		const visualContext = chatSessionTask.value === "support" ? await getVisualContext(workflowId, nodeInfo) : { aiUsageSettings: { allowSendingParameterValues: allowSendingParameterValues.value } };
		...
		chatWithAssistant(rootStore.restApiContext, { payload }, (msg) => onEachStreamingMessage(msg, id), () => onDoneStreaming(id), (e) => handleServiceError(e, id, async () => await initSupportChat(workflowId, userMessage, credentialType)), streamingAbortController.value.signal);
	}
	async function initErrorHelper(workflowId, context) {
		const id = getRandomId();
		...
		currentSessionWorkflowId.value = workflowId;
		const activeExecutionId = useWorkflowExecutionStateStore(createWorkflowDocumentId(workflowsStore.workflowId)).activeExecutionId;
		if (activeExecutionId) currentSessionActiveExecutionId.value = activeExecutionId;
		const { authType, nodeInputData, schemas } = assistantHelpers.getNodeInfoForAssistant(workflowId, context.node, { excludeParameterValues: !allowSendingParameterValues.value });
		...
		if (streamingAbortController.value) streamingAbortController.value.abort();
		streamingAbortController.value = new AbortController();
		chatWithAssistant(rootStore.restApiContext, { payload }, (msg) => onEachStreamingMessage(msg, id), () => onDoneStreaming(id), (e) => handleServiceError(e, id, async () => await initErrorHelper(workflowId, context)), streamingAbortController.value.signal);
	}
	...
	async function sendMessage(workflowId, chatMessage) {
		if (isSessionEnded.value || streaming.value) return;
		const id = getRandomId();
		const retry = async () => {
		...
		try {
			...
			const activeNode = ndvStore.value.activeNode;
			const userContext = await getVisualContext(workflowId, assistantHelpers.getNodeInfoForAssistant(workflowId, activeNode, { excludeParameterValues: !allowSendingParameterValues.value }));
			if (streamingAbortController.value) streamingAbortController.value.abort();
			streamingAbortController.value = new AbortController();
			chatWithAssistant(rootStore.restApiContext, {
				payload: {
	...
	function trackUserOpenedAssistant({ workflowId, source, task, has_existing_session }) {
		const canvasStatus = useWorkflowDocumentStore(createWorkflowDocumentId(workflowId)).allNodes.length === 0 ? "empty" : "existing_workflow";
	...
	watch(() => useWorkflowExecutionStateStore(createWorkflowDocumentId(workflowsStore.workflowId)).activeExecutionResultDataLastUpdate, () => {
