...
import { $ as openBlock, A as createTextVNode, C as createBaseVNode, Cn as toDisplayString, E as createElementBlock, F as guardReactiveProps, Gt as unref, H as mergeModels, It as ref, N as defineComponent, S as computed, T as createCommentVNode, X as onMounted, _ as Fragment, bt as withCtx, dt as useModel, j as createVNode, rt as renderList, vn as normalizeClass, w as createBlock, yn as normalizeProps } from "./vue.runtime.esm-bundler-Bs4WIMNP.js";
import { c as I18nT, s as useI18n } from "./src-CYuRIBuX.js";
import { E as N8nUserInfo_default, Ga as N8nHeading_default, It as Alert_default, Ja as N8nButton_default, Ki as N8nActionBox_default, Lt as N8nActionToggle_default, Rt as N8nActionDropdown_default, Ui as Input_default, W as N8nNotice_default, Ya as N8nIcon_default, b as N8nDataTableServer_default, lt as N8nLink_default, qa as N8nText_default, zi as N8nTooltip_default } from "./src-CSWS3PCK.js";
...
import { Ed as EnterpriseEditionFeature, Id as getDebounceTime, Jo as USERS_LIST_SORT_OPTIONS, Od as DEBOUNCE_TIME, Za as ROLE, ui as VIEWS } from "./constants-U-laJZAl.js";
import { t as useSettingsStore } from "./settings.store-CgTGI66t.js";
import { Tt as INVITE_USER_MODAL_KEY, ct as DELETE_USER_MODAL_KEY, f as useUIStore, n as hasPermission, t as useUsersStore } from "./users.store-Bf6s0cNu.js";
...
import { r as formatTimeAgo } from "./dateFormatter-BsYMNC2w.js";
import { t as useRolesStore } from "./roles.store-BD9FWnSU.js";
import { n as useSSOStore } from "./sso.store-D8gCI_5i.js";
import { n as CustomRolesUpgradeModal_default, t as RoleSelectDropdown_default } from "./RoleSelectDropdown-CR-WH_S_.js";
import { t as useUserRoleProvisioningStore } from "./userRoleProvisioning.store-BiBnjEJL.js";
import { i as countGrantedInstancePermissions, r as TOTAL_INSTANCE_PERMISSIONS } from "./instanceRoleScopes-Dsy4RjAb.js";
...
var SettingsUsersRoleCell_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		const customInstanceRolesEnabled = computed(() => envFeatureFlagCheck.value("CUSTOM_INSTANCE_ROLES"));
		...
		const legacyRoleLabels = computed(() => ({
			[ROLE.Owner]: {
		...
		const legacyRoleLabel = computed(() => legacyRoleLabels.value[currentRole.value]?.label ?? selectedRole.value?.displayName);
		const legacyRoleActions = computed(() => [
			{
				id: ROLE.Member,
				label: i18n.baseText("auth.roles.member")
			},
		...
		const onLegacyActionSelect = (role) => {
			emit("update:role", {
				role,
				userId: props.data.id
			});
		...
		return (_ctx, _cache) => {
			return customInstanceRolesEnabled.value ? (openBlock(), createElementBlock("div", {
				key: 0,
				class: normalizeClass(_ctx.$style.flagBranch)
			}, [isEditable.value ? (openBlock(), createBlock(RoleSelectDropdown_default, {
				key: 0,
				...
				"is-admin-or-owner": isAdminOrOwner.value,
				...
				"view-route-name",
				"from-view"
			])) : (openBlock(), createElementBlock("span", _hoisted_1$1, toDisplayString(selectedRole.value?.displayName), 1)), createVNode(CustomRolesUpgradeModal_default, {
				modelValue: upgradeModalVisible.value,
				"onUpdate:modelValue": _cache[1] || (_cache[1] = ($event) => upgradeModalVisible.value = $event)
			}, null, 8, ["modelValue"])], 2)) : (openBlock(), createElementBlock("div", _hoisted_2, [isEditable.value ? (openBlock(), createBlock(unref(N8nActionDropdown_default), {
				key: 0,
				placement: "bottom-start",
				items: legacyRoleActions.value,
				disabled: __props.loading,
				"data-test-id": "user-role-dropdown",
				onSelect: onLegacyActionSelect
			}, {
				activator: withCtx(() => [createBaseVNode("button", {
					...
					default: withCtx(() => [createTextVNode(toDisplayString(legacyRoleLabel.value), 1)]),
					_: 1
				}), __props.loading ? (openBlock(), createBlock(unref(N8nIcon_default), {
					key: 0,
					color: "text-dark",
					icon: "spinner",
					spin: "",
				...
				menuItem: withCtx((item) => [createVNode(unref(ElRadio), {
					...
					default: withCtx(() => [createBaseVNode("span", { class: normalizeClass(_ctx.$style.radioLabel) }, [createVNode(unref(N8nText_default), {
						...
						default: withCtx(() => [createTextVNode(toDisplayString(legacyRoleLabels.value[item.id]?.desc), 1)]),
						_: 2
					}, 1024)], 2)]),
					_: 2
				}, 1032, ["model-value", "label"])]),
				_: 1
			}, 8, ["items", "disabled"])) : (openBlock(), createElementBlock("span", _hoisted_4, toDisplayString(legacyRoleLabel.value), 1))]));
		};
	}
});
var SettingsUsersRoleCell_vue_vue_type_style_index_0_lang_module_default = {
	flagBranch: "_flagBranch_1hleo_126",
	roleLabel: "_roleLabel_1hleo_131",
	radioLabel: "_radioLabel_1hleo_145"
};
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", { class: normalizeClass(_ctx.$style.projects) }, [
				...
				projects.value.additional.length > 0 ? (openBlock(), createBlock(unref(N8nTooltip_default), { key: 1 }, {
					content: withCtx(() => [createBaseVNode("ul", { class: normalizeClass(_ctx.$style.projectList) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(projects.value.additional, (project, index) => {
						return openBlock(), createElementBlock("li", { key: index }, toDisplayString(project), 1);
					}), 128))], 2)]),
					default: withCtx(() => [createBaseVNode("span", { class: normalizeClass(_ctx.$style.project) }, "+ " + toDisplayString(projects.value.additional.length), 3)]),
					_: 1
	...
	setup(__props) {
		...
		const userRoles = computed(() => {
			return [
				...
				...customInstanceRolesEnabled.value ? rolesStore.customInstanceRoles.map((role) => ({
					value: role.slug,
					label: role.displayName,
		...
		async function onReinvite(userId) {
			try {
				...
				if (user?.email && user?.role) {
					if (!(customInstanceRolesEnabled.value ? user.role !== ROLE.Owner && user.role !== ROLE.Default && user.role !== ROLE.ChatUser : [ROLE.Admin, ROLE.Member].includes(user.role))) throw new Error("Invalid role name on reinvite");
...
var SettingsUsersView_vue_vue_type_style_index_0_lang_module_default = {
	userCount: "_userCount_17a4t_125",
	buttonContainer: "_buttonContainer_17a4t_130",
	search: "_search_17a4t_137",
	setupInfoContainer: "_setupInfoContainer_17a4t_141",
	container: "_container_17a4t_145"
};
