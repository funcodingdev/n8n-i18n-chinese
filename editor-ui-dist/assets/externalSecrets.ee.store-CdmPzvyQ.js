...
import { Ac as getGlobalExternalSecrets, Dc as getExternalSecrets, Ec as connectProvider, Mc as reloadProvider, Nc as testExternalSecretsProviderConnection, Oc as getExternalSecretsProvider, Pc as updateProvider, jc as getProjectExternalSecrets, js as useSettingsStore, kc as getExternalSecretsProviders, wr as useRBACStore } from "./users.store-BPj3XTmq.js";
...
const useExternalSecretsStore = defineStore("externalSecrets", () => {
	...
	async function fetchGlobalSecrets() {
		if (rbacStore.hasScope("externalSecret:list")) try {
			...
			state.secrets = moduleConfig?.forProjects || moduleConfig?.multipleConnections ? await getGlobalExternalSecrets(rootStore.restApiContext) : await getExternalSecrets(rootStore.restApiContext);
		} catch {
			state.secrets = {};
		}
	}
