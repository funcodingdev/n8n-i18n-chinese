...
import { v as i18n } from "./_MapCache-DTYzu2mo.js";
import { $n as makeRestApiRequest, dr as defineStore, t as useRootStore } from "./useRootStore-D6sIVZY4.js";
...
import { n as COMPUTER_USE_CONNECTION_TYPE, t as BROWSER_USE_CONNECTION_TYPE } from "./constants-Dut2hF19.js";
...
function deriveInstanceAiConfiguration(settings, modelCredentials, serviceCredentials) {
	...
	const modelConfigured = Boolean(settings?.modelEnvConfigured || settings?.modelCredentialId && settings.modelName);
	const sandboxCredentialId = settings?.sandboxProvider === "daytona" ? settings.daytonaCredentialId : settings?.n8nSandboxCredentialId;
	const sandboxConfigured = Boolean(settings?.sandboxEnabled && (sandboxCredentialId || settings.sandboxEnvConfigured));
	const searchCredential = serviceCredentials.find((credential) => credential.id === settings?.searchCredentialId);
	let searchState = "notset";
	...
	return {
		...
		searchCredential,
		searchState,
		setupCompleted: modelConfigured && sandboxConfigured && searchState !== "notset",
