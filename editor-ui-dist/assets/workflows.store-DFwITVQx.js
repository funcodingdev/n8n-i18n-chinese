const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/workers-DWaV3A26.js","assets/comlink-BtXgpLdT.js"])))=>i.map(i=>d[i]);
...
import { i as i18n, n as addHeaders, r as addNodeTranslation, s as useI18n } from "./src-80JFTwhd.js";
	...
	const loadNodesInformationFromLocalDb = async (nodeInfos) => {
		try {
			const { getNodeType } = await __vitePreload(async () => {
				const { getNodeType } = await import("./workers-DWaV3A26.js");
				return { getNodeType };
			}, __vite__mapDeps([0,1]));
			const found = [];
			const missing = [];
	...
	const loadNodeTypesFromLocalDb = async () => {
		try {
			const { loadNodeTypes, getAllNodeTypes } = await __vitePreload(async () => {
				const { loadNodeTypes, getAllNodeTypes } = await import("./workers-DWaV3A26.js");
				return {
					loadNodeTypes,
					getAllNodeTypes
				};
