...
import { E as useWorkflowDocumentStore, S as createWorkflowDocumentId, t as useWorkflowsStore, vt as useCredentialsStore, y as useWorkflowExecutionStateStore } from "./workflows.store-YK4BoC2f.js";
import { i as defineStore, t as STORES } from "./constants-Box1qD-P.js";
import { $i as v4, A as chatHubMessageWithButtonsSchema, E as VECTOR_STORE_PROVIDER_CREDENTIAL_TYPE_MAP, M as chatHubVectorStoreProviderSchema, N as emptyChatModelsResponse, T as PROVIDER_CREDENTIAL_TYPE_MAP, Wn as createRunExecutionData, fi as deepCopy, k as chatHubLLMProviderSchema } from "./src-DVvN4F-B.js";
import { Xn as makeRestApiRequest, qn as ResponseError, t as useRootStore } from "./useRootStore-Bd5Zf8xA.js";
import { t as useSettingsStore } from "./settings.store-CYQk6rCS.js";
import "./settings.store-iCnxkLnT.js";
import { N as IN_PROGRESS_EXECUTION_ID, Pt as CHAT_TRIGGER_NODE_TYPE, po as EnterpriseEditionFeature } from "./constants-CQOOvzhs.js";
import { n as useTelemetry } from "./posthog.store-jf7EASbZ.js";
import { t as retry } from "./retry-z3HauhxE.js";
import { T as unflattenModel, a as createSessionFromStreamingState, i as createFakeAgent, l as flattenModel, n as chunkFilesBySize, r as createAiMessageFromStreamingState, t as buildUiMessages, y as isMatchedAgent } from "./chat.utils-BzoF5HNa.js";
import { t as convertFileToBinaryData } from "./fileUtils-CS1DhYzy.js";
...
var updateChatEnabledApi = async (context, enabled) => {
	return await makeRestApiRequest(context, "PUT", "/chat/enabled", { enabled });
};
...
var useChatStore = defineStore(STORES.CHAT_HUB, () => {
	...
	*/
	function isCanvasManualExecution(model, canvasWorkflowId) {
		if (model.provider !== "n8n") return false;
		if (canvasWorkflowId !== model.workflowId) return false;
		const chatTrigger = useWorkflowDocumentStore(createWorkflowDocumentId(canvasWorkflowId)).allNodes.find((node) => node.type === CHAT_TRIGGER_NODE_TYPE);
	...
	*/
	function initManualExecutionScaffold(workflowId) {
		const workflowsStore = useWorkflowsStore();
		const workflowDocumentStore = useWorkflowDocumentStore(createWorkflowDocumentId(workflowId));
		useWorkflowExecutionStateStore(createWorkflowDocumentId(workflowsStore.workflowId)).setWorkflowExecutionData({
		...
		useWorkflowExecutionStateStore(createWorkflowDocumentId(workflowsStore.workflowId)).setActiveExecutionId(null);
	}
	async function sendMessage(sessionId, message, agent, credentials, files = [], canvasWorkflowId) {
		const messageId = v4();
		const conversation = ensureConversation(sessionId);
		...
		const useManualMode = !!canvasWorkflowId && isCanvasManualExecution(agent.model, canvasWorkflowId);
		...
		try {
			...
			if (useManualMode) {
				...
				await sendMessageManualApi(rootStore.restApiContext, agent.model.workflowId, {
					messageId,
					sessionId,
					message,
					previousMessageId,
	...
	async function editMessage(sessionId, editId, content, agent, credentials, keepAttachmentIndices = [], newFiles = [], canvasWorkflowId) {
		const promptId = v4();
	...
	async function regenerateMessage(sessionId, retryId, agent, credentials, canvasWorkflowId) {
